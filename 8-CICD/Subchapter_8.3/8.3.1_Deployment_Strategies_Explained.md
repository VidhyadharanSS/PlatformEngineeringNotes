...
> **Analogy:** Imagine a theater with two identical stages. The audience watches one stage (Blue). Meanwhile, actors rehearse the new show on the other stage (Green). When ready, you simply redirect the audience to the Green stage — instant switchover, and if something goes wrong, you redirect them back.
...
> **⚠️ Advanced (skip on first pass):** Argo Rollouts is production-grade tooling. Understand basic canary concepts (above) first, then come back here.

Manually adjusting Istio/Nginx weights (1% → 5% → 20% → 100%) is tedious and error-prone. **Argo Rollouts** is a Kubernetes-native progressive delivery controller that automates the entire canary lifecycle — including metric analysis and automatic rollback.
...
```yaml
# Argo Rollout resource — replaces standard Kubernetes Deployment
...
spec:
  ...
  strategy:
    canary:
      ...
      # If any analysis fails → automatic rollback to stable version
      canaryService: myapp-canary    # Kubernetes Service for canary pods
      stableService: myapp-stable    # Kubernetes Service for stable pods
      trafficRouting:
        istio:
          virtualService:
            name: myapp-vsvc
  ...
  template:
    ...
    spec:
      containers:
        - name: myapp
          image: ghcr.io/org/myapp:v2.0.0
```
...
flowchart LR
    PUSH(["📝 New image\npushed"]) --> R5["5% canary\n⏱️ 5 min pause"]
    R5 --> A1["📊 Analysis\nsuccess rate ≥ 95%?"]
    A1 -->|"✅ Pass"| R25["25% canary\n⏱️ 10 min"]
    A1 -->|"❌ Fail"| ROLLBACK(["🔙 Auto-rollback\nto stable version"])
    R25 --> A2["📊 Analysis"]
    A2 -->|"✅ Pass"| R50["50% → 100%\nFull rollout ✅"]
    A2 -->|"❌ Fail"| ROLLBACK

    style R5 fill:#FF9800,color:#fff
    style R50 fill:#4CAF50,color:#fff
    style ROLLBACK fill:#f44336,color:#fff
```

> **Argo Rollouts vs raw Istio VirtualService:** With raw Istio, you manually edit weights in your pipeline (`kubectl patch virtualservice`). With Argo Rollouts, you declare the desired progression in the `Rollout` spec and it handles weight changes, metric analysis, and rollback automatically. Use Argo Rollouts in production; use raw Istio for learning the concepts.
