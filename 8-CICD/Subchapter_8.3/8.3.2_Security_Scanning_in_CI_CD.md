...
> **Analogy:** Think of a book being published. It's cheap to fix a typo while drafting (development), but once the book is printed and in stores (production), you need a full recall. Security scanning in CI is like having proofreaders check every draft automatically.
...
# ── The --ignore-unfixed flag ──────────────────────────────────────────────────
# Show only CVEs that have a fix available (skip "no fix yet" findings)
trivy fs --ignore-unfixed --severity CRITICAL,HIGH .

# Container images: ignore CVEs with no patched version
trivy image --ignore-unfixed --severity CRITICAL myapp:latest
```

> **Why `--ignore-unfixed` matters:** A typical base image scan may find 200+ CVEs, but 150 of them have **no fix available** — the upstream OS vendor hasn't released a patch yet. Without `--ignore-unfixed`, teams get overwhelmed by findings they cannot act on, leading to "alert fatigue" where real actionable CVEs are buried in noise. **Best practice in CI:** use `--ignore-unfixed` for pipeline-blocking scans (exit-code 1), and run a separate nightly scan **without** the flag to track all known vulnerabilities for reporting.

```yaml
# GitHub Actions: two-tier Trivy scanning
- name: Actionable scan (blocks pipeline)
  uses: aquasecurity/trivy-action@master
  with:
    scan-type: 'fs'
    severity: 'CRITICAL,HIGH'
    ignore-unfixed: true       # only fixable CVEs
    exit-code: '1'             # fail if any found

- name: Full scan (reporting only — does not block)
  uses: aquasecurity/trivy-action@master
  with:
    scan-type: 'fs'
    severity: 'CRITICAL,HIGH,MEDIUM'
    ignore-unfixed: false      # all CVEs including unfixed
