...
> **Important:** CI and CD are often mixed up. **CI** = every commit is built + tested automatically. **CD** (Delivery) = every passing build is *ready* to deploy. **CD** (Deployment) = every passing build is *automatically* deployed. Most teams practise CI + Continuous Delivery; only the most mature ones practise full Continuous Deployment to production.

> **Tip:** A slow pipeline defeats the purpose. Aim for < 10 min feedback on PRs — parallelise tests, cache dependencies, and run heavy integration suites only on main.
...
> **⚠️ Advanced (skip on first pass):** DORA metrics are interview material and team maturity indicators. Come back after you've built your first pipeline — the metrics will make more sense once you've experienced the workflow.
...
> **Note:** You don't need to learn all these tools now — Module 8 focuses on GitHub Actions. This table is for reference. GitLab CI and Jenkins are covered in 8.3.3.
...
> **Analogy:** Think of a webhook like a doorbell — when someone pushes code, GitHub rings the CI server's doorbell to say "come check the new delivery." The CI server then pulls the code and starts the pipeline.
