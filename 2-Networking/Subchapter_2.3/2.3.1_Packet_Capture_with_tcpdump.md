...
# ⚠️ Advanced: Capture only HTTP GET requests using byte-offset filters
# (You'll rarely need this — the simple 'port 80' filter above is usually enough)
sudo tcpdump -i eth0 -n -A 'port 80 and tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x47455420'
# 0x47455420 = "GET " in hex (matches the literal bytes G-E-T-space in the TCP payload)

# ⚠️ Advanced: Capture only HTTP 200 responses
sudo tcpdump -i eth0 -n -A 'port 80 and tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x48545450'
# 0x48545450 = "HTTP" in hex
```
...
### Example 6: Capturing a Complete TCP Handshake (Step-by-Step)

This is the most important capture to understand — seeing the 3-way handshake in real packets:

```bash
# Terminal 1: Start capturing SYN/FIN packets
sudo tcpdump -i eth0 -n 'host example.com and port 80 and (tcp[tcpflags] & (tcp-syn|tcp-fin) != 0)' -c 6
...
10:00:01.001 IP 192.168.1.100.54321 > 93.184.216.34.80: Flags [S], seq 100, win 65535

# 2. Server → Client: SYN-ACK (OK, I accept)
10:00:01.025 IP 93.184.216.34.80 > 192.168.1.100.54321: Flags [S.], seq 300, ack 101, win 29200

# 3. Connection established, data flows...

# 4. Client → Server: FIN (I'm done)
10:00:01.090 IP 192.168.1.100.54321 > 93.184.216.34.80: Flags [F.], seq 89, ack 1256

# 5. Server → Client: FIN-ACK (OK, I'm done too)
10:00:01.115 IP 93.184.216.34.80 > 192.168.1.100.54321: Flags [F.], seq 1256, ack 90
```
...
| --------------------------------------- | ------------------------------------- |
| `[S]` sent, no `[S.]` back             | Firewall dropping SYN (timeout)       |
| `[S]` sent, `[R]` back                 | Port closed or firewall rejecting     |
| `[S]` retransmitted 3-5 times          | Packet loss or silent DROP            |
| Many `[R]` resets from server           | Server overloaded, backlog full       |
| Handshake completes but `[R]` after data| Application-level rejection           |

### Example 7: Tcpdump on Docker Containers and Kubernetes Pods (Preview — Modules 4 & 5)

> **Note:** This section uses Docker and Kubernetes concepts covered in Modules 4 and 5. Bookmark this and return after learning containers. The tcpdump skills you've learned above still apply — the only difference is *where* you run tcpdump (inside a container's network namespace).

In containerized environments, you often need to capture traffic inside a network namespace:

```bash
# Docker: capture on a container's virtual interface
# Find container's PID
CONTAINER_PID=$(docker inspect --format '{{.State.Pid}}' my_container)

# Enter network namespace and run tcpdump
sudo nsenter -t $CONTAINER_PID -n tcpdump -i eth0 -n port 80 -c 20

# Alternative: run tcpdump inside the container (if tcpdump installed)
docker exec my_container tcpdump -i eth0 -n -c 20

# Kubernetes: capture inside a pod
kubectl exec -it my-pod -- tcpdump -i eth0 -n port 8080 -c 50

# If tcpdump not in pod, use ephemeral debug container (K8s 1.23+)
kubectl debug -it my-pod --image=nicolaka/netshoot --target=my-container -- tcpdump -i eth0 -n
```

> **Tip:** The `nicolaka/netshoot` image includes tcpdump, curl, dig, nmap, and dozens of other networking tools — essential for debugging K8s network issues.
