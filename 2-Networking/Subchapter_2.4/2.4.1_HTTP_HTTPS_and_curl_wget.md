...
> **Tip:** `curl` is your quickest truth source. Browsers hide redirects, cache, cookies, and CORS behavior; `curl -v` shows the raw HTTP conversation.
...
> **Warning:** "safe" does **not** mean "harmless" operationally. A `GET` can still be expensive if it triggers huge database reads, cache misses, or large downloads.
...
### HTTP/1.1 vs HTTP/2 vs HTTP/3 (What's Different)

> **For beginners:** HTTP/1.1 is the foundation — everything you learn about methods, headers, and status codes applies equally to HTTP/2 and HTTP/3. The newer versions are performance improvements (how data travels on the wire) rather than fundamental changes to how HTTP works. Focus on HTTP/1.1 first.

| Feature              | HTTP/1.1                          | HTTP/2                | HTTP/3                |
| -------------------- | --------------------------------- | --------------------- | --------------------- |
| Transport protocol   | TCP                               | TCP                   | **QUIC (UDP-based)**  |
| Connection model     | Multiple parallel TCP connections | Single TCP connection | Single QUIC connection |
| Request multiplexing | No (requests block)               | Yes (interleaved)     | Yes (no head-of-line blocking) |
| Header compression   | No                                | Yes (HPACK)           | Yes (QPACK)           |
| Server push          | No                                | Yes                   | Yes                   |
| Binary vs text       | Text                              | Binary                | Binary                |
| Connection setup     | TCP + TLS (2 RTT)                 | TCP + TLS (2 RTT)     | **0-RTT or 1-RTT**    |
| Connection migration | No                                | No                    | **Yes (IP changes OK)** |
...
- **Connection migration**: Mobile devices can change networks without reconnecting

```bash
# Check if server supports HTTP/3
curl --http3 https://cloudflare.com -I 2>&1 | grep -i alt-svc
# alt-svc: h3=":443"

# Force HTTP/3 (curl 7.66+ with HTTP/3 support)
...
**Note:** HTTP/3 support requires curl compiled with HTTP/3 libraries (nghttp3, ngtcp2).

***

## Part 2b: HTTP Caching Headers

Caching is one of the most impactful performance optimizations. Understanding cache headers is critical for debugging "stale content" issues.

| Header             | Direction | Purpose                                   | Example                          |
| ------------------ | --------- | ----------------------------------------- | -------------------------------- |
| `Cache-Control`    | Both      | Primary caching directive                 | `Cache-Control: max-age=3600`    |
| `ETag`             | Response  | Unique content fingerprint                | `ETag: "abc123"`                 |
| `If-None-Match`    | Request   | Conditional request (compare ETag)        | `If-None-Match: "abc123"`        |
| `Last-Modified`    | Response  | When content was last changed             | `Last-Modified: Tue, 01 Jan ...` |
| `If-Modified-Since`| Request   | Conditional request (compare date)        | `If-Modified-Since: Tue, 01...`  |
| `Expires`          | Response  | Legacy expiration date (use Cache-Control)| `Expires: Thu, 01 Jan 2025 ...`  |

**Common `Cache-Control` directives:**

```bash
# Cacheable for 1 hour (shared caches like CDNs + browser)
Cache-Control: public, max-age=3600

# Cacheable only by browser (not CDNs)
Cache-Control: private, max-age=600

# Never cache (API responses, user-specific data)
Cache-Control: no-store

# Cache but revalidate every time (check ETag)
Cache-Control: no-cache

# Force revalidation after stale
Cache-Control: must-revalidate, max-age=300
```

```bash
# Check cache headers with curl
curl -I https://example.com/style.css | grep -i "cache-control\|etag\|expires"

# Simulate conditional request (304 Not Modified if unchanged)
curl -I -H "If-None-Match: \"abc123\"" https://example.com/api/data
# HTTP/1.1 304 Not Modified  ← no body transferred, saves bandwidth
```

> **Tip:** When debugging "users see old content after deploy", check `Cache-Control` headers. Use cache-busting filenames (e.g., `style.abc123.css`) for static assets.
...
# Environment variable proxy (used by many tools)
...
**What is "cross-origin"?** When a web page at `https://frontend.example.com` tries to make an API call to `https://api.different-site.com`, that's a cross-origin request — the two URLs have different "origins" (different domain names). Browsers block these by default as a security measure to prevent malicious websites from stealing data from other sites you're logged into.

CORS is the mechanism that allows servers to opt-in to receiving cross-origin requests. As a platform engineer, you'll debug CORS errors when frontends can't reach backends that are on different domains or subdomains.

```bash
# Simulate a CORS preflight request (browser sends OPTIONS first)
...
# Check response headers (must include these for CORS to work):
...
> **Tip:** If the browser shows "CORS policy" errors but `curl` works fine — that's expected. CORS is a **browser** security mechanism. `curl` doesn't enforce CORS. The fix is always on the **server** side (add CORS headers).
...
- [2.1.1 OSI and TCP/IP Models](../Subchapter_2.1/2.1.1_OSI_and_TCP_IP_Models.md) – TCP/UDP (layer 4 protocols), port numbers (HTTP=80, HTTPS=443)
- [2.1.2 IP Addressing and Subnetting](../Subchapter_2.1/2.1.2_IP_Addressing_Subnetting_CIDR.md) – Virtual IPs, backend server addressing
- [2.3.2 Firewalling with iptables/nftables](../Subchapter_2.3/2.3.2_Firewalling_with_iptables_nftables.md) – NAT and load balancer interaction

**Next note:** [2.4.2 Load Balancing Layer 4 vs Layer 7](./2.4.2_Load_Balancing_Layer4_vs_Layer7.md)
