...
## Part 8: CAA Records and DNSSEC (Advanced — Security Hardening)

> **For beginners:** CAA and DNSSEC are security-hardening features. You don't need them for basic DNS troubleshooting. Learn Parts 1–7 first, then come back here when you're managing production domains or preparing for security reviews.

### CAA Records — Certificate Authority Authorization

CAA records specify which Certificate Authorities (CAs) are allowed to issue certificates for your domain. This prevents unauthorized certificate issuance.
...
| Tag        | Purpose                                          | Example                            |
| ---------- | ------------------------------------------------ | ---------------------------------- |
| `issue`    | Allow CA to issue certificates                   | `0 issue "letsencrypt.org"`        |
| `issuewild`| Allow CA to issue wildcard certificates          | `0 issuewild "digicert.com"`       |
| `iodef`    | Report unauthorized issuance attempts            | `0 iodef "mailto:sec@example.com"` |

> **Tip:** If no CAA record exists, any CA can issue certificates for your domain. Always set CAA for production domains.
...
dig @8.8.8.8 example.com +dnssec | grep -i "ad"
# flags: ... ad;   ← AD (Authenticated Data) flag means DNSSEC validated

# Check DNSKEY record
dig example.com DNSKEY +short

# Check DS (Delegation Signer) at parent zone
...
| `DS`     | Hash of child zone's DNSKEY (at parent)  |
| `NSEC/3` | Proves non-existence of a domain         |

**When DNSSEC matters:** DNSSEC protects against DNS cache poisoning (e.g., an attacker redirecting `bank.com` to a phishing IP). It does NOT encrypt DNS traffic — that's DNS-over-HTTPS (DoH) or DNS-over-TLS (DoT).

***

## Part 9: DNS Caching and TTL
