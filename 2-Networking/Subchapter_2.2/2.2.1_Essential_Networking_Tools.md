...
> **Learning priority:**
> - **Essential (learn now):** `ping`, `traceroute`/`mtr`, `ss`, `ip addr`/`ip route` — you'll use these daily.
> - **Important (learn soon):** `arp`, `lsof -i`, `netcat` — needed for deeper troubleshooting.
> - **Advanced (learn when needed):** `nmap`, `iftop`/`nethogs`, `sar` — specialized tools for specific scenarios.

> **Tip:** Follow a fixed sequence under pressure: **name resolution → IP reachability → route → port/listener → application response**. Consistency prevents random guessing during outages.
...
> **Warning:** Flood ping can generate high traffic and trigger monitoring alarms or rate limits. Use it only in controlled environments.
...
## Part 6: Bandwidth Monitoring — iftop and nethogs

When you need to see **who** is consuming bandwidth in real-time, `iftop` and `nethogs` are indispensable.
...
sudo dnf install iftop     # RHEL/Rocky
...
sudo iftop -i eth0 -f "host 10.0.0.5"
```

**Interactive keys:** `t` toggle display, `s`/`d` show source/dest, `p` toggle ports, `q` quit.
...
sudo dnf install nethogs   # RHEL/Rocky

# Show bandwidth per process (most useful!)
sudo nethogs eth0

# Output:
# PID    USER     PROGRAM              DEV     SENT    RECEIVED
# 1234   www      /usr/sbin/nginx      eth0    15.2    340.5  KB/sec
# 5678   app      python3 app.py       eth0    8.1     12.3   KB/sec
```
...
| Tool       | Shows                                   | Best For                          |
| ---------- | --------------------------------------- | --------------------------------- |
| `iftop`    | Bandwidth per connection (IP pairs)     | Finding which host is flooding    |
| `nethogs`  | Bandwidth per process (PID)             | Finding which app is using BW     |
| `nload`    | Total interface bandwidth (graph)       | Quick overall throughput check    |
| `sar -n DEV` | Historical interface stats           | Post-incident analysis            |

### Quick nmap Port Scanning

> **⚠️ Security & Ethics Warning:** NEVER scan hosts you don't own or have explicit authorization to test. Unauthorized port scanning is illegal in many jurisdictions and may trigger security alerts. Only use nmap on your own infrastructure, lab environments, or targets you have written permission to scan.

`nmap` is the standard tool for discovering open ports on remote hosts:
...
# Scan port range
nmap -p 1-65535 192.168.1.100    # full scan (slow)

# TCP SYN scan (fast, stealthy, needs root)
sudo nmap -sS 192.168.1.100

# UDP scan (slow but important for DNS, DHCP)
...
> **Tip:** Never scan networks you don't own without explicit permission. Port scanning without authorization may violate laws.
