...
> **Tip:** Use the models as a **troubleshooting lens**, not as theory to memorize blindly. In real incidents, your goal is to quickly narrow the failing layer and pick the right tool.
...
> **Important:** Encapsulation explains why packet captures look noisy. A single HTTP request includes Ethernet, IP, and TCP metadata around the application payload.
...
## Part 6: TCP 3-Way Handshake and Connection States

### Beginner Bridge: What Happens When You Run `curl https://api.example.com`

This is the shortest practical story of networking. Keep this sequence in your head during incidents:

| Step | Layer | What Happens | Failure Looks Like | First Check |
|---|---|---|---|---|
| 1 | DNS / Layer 7 | Name resolves to an IP address | `Could not resolve host` | `dig api.example.com` |
| 2 | Routing / Layer 3 | Your host decides the next hop | `Network is unreachable`, `No route to host` | `ip route get <ip>` |
| 3 | Firewall / Layer 3-4 | Rules allow or block traffic | Timeout, reset, or refused | `sudo iptables -S`, `sudo nft list ruleset` |
| 4 | TCP / Layer 4 | Client and server complete SYN, SYN-ACK, ACK | Timeout or connection refused | `nc -zv api.example.com 443` |
| 5 | TLS / Layer 6-7 | Certificate is verified and encryption starts | Certificate or handshake error | `openssl s_client -connect host:443 -servername host` |
| 6 | HTTP / Layer 7 | Request path, headers, and body are handled | 4xx/5xx status code | `curl -v https://api.example.com` |

The important lesson: **same symptom, different layer**. A timeout usually means packets are not returning. `Connection refused` means the server replied but nothing is listening on that port. A `404` means networking worked and the application rejected the path.

***

Understanding TCP connection establishment and teardown is essential for debugging connection issues in production.
...
Client                    Server
  ...
  |<-- SYN-ACK (seq=300,     |   Step 2: Server acknowledges + sends its SYN
  |     ack=101) ------------|
  |--- ACK (ack=301) ------->|   Step 3: Client acknowledges
  |                           |   Connection ESTABLISHED
```

**Practical capture of a handshake:**

```bash
# Preview: tcpdump is covered in detail in 2.3.1. For now, just know this captures
# TCP connection setup/teardown packets. You'll learn the filter syntax later.
sudo tcpdump -i eth0 -n 'tcp[tcpflags] & (tcp-syn|tcp-fin) != 0' -c 10
# You'll see [S], [S.], then data begins
```

### TCP Connection States (What `ss` Shows)

| State         | Meaning                                         | Common Issue                    |
| ------------- | ----------------------------------------------- | ------------------------------- |
| `LISTEN`      | Waiting for incoming connections                | Expected for servers            |
| `SYN_SENT`    | Client sent SYN, awaiting SYN-ACK              | Firewall blocking outbound      |
| `SYN_RECV`    | Server received SYN, sent SYN-ACK              | SYN flood attack                |
| `ESTABLISHED` | Connection active and healthy                   | Normal                          |
| `TIME_WAIT`   | Connection closed, waiting before port reuse    | **Too many = port exhaustion**  |
| `CLOSE_WAIT`  | Remote closed, local app hasn't closed yet      | **Application bug (FD leak)**   |
| `FIN_WAIT1/2` | Local side initiated close, awaiting completion | Normal teardown in progress     |

```bash
# Count connections by state (critical for debugging)
ss -tan | awk '{print $1}' | sort | uniq -c | sort -rn
#   450 ESTAB
#   120 TIME-WAIT
#     8 CLOSE-WAIT    ← if growing, app is leaking connections
#     2 LISTEN

# Find processes stuck in CLOSE_WAIT
ss -tanp state close-wait

# Check TIME_WAIT accumulation (common under high load)
ss -tan state time-wait | wc -l
```

> **Warning:** A growing `CLOSE_WAIT` count means your application is not closing sockets properly — this is a code bug. A high `TIME_WAIT` count is normal under load but can cause port exhaustion. Tune with `net.ipv4.tcp_tw_reuse=1` (a kernel parameter set via `sysctl` — covered in [1.1.1](../../1-Linux/Subchapter_1.1/1.1.1_Kernel_OS_and_Distros.md)).
...
Client                    Server
  ...
  |<-- FIN -------------------|   Step 3: Server finishes and closes
  |--- ACK ------------------>|   Step 4: Client acknowledges
  |   (TIME_WAIT 2*MSL)      |   Client waits before freeing port
```

***

## Part 7: Common Port Numbers to Memorize
...
- [1.1.3 CLI Fundamentals](../../1-Linux/Subchapter_1.1/1.1.2_CLI_Basics_and_Philosophy.md) – Command-line basics (`ping`, `telnet`, `nc`)
