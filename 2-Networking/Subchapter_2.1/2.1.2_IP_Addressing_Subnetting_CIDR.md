...
### Example 2: AWS VPC Subnet Design (Advanced — Cloud)

> **For beginners:** This example uses AWS cloud terminology (VPCs, Availability Zones, NAT gateways). If you haven't worked with cloud infrastructure yet, skip to Part 8. The subnetting math is the same — only the context is cloud-specific.
...
MTU is the largest packet size that can be transmitted without fragmentation. Default Ethernet MTU is **1500 bytes**.

| MTU Size | Context | Notes |
|----------|---------|-------|
| 1500 | Standard Ethernet | Default for most networks |
| 9000 | Jumbo frames | For high-performance networks (requires switch support) |
| 1400 | VPN/tunnels | Reduced to account for encapsulation overhead |
| 576 | Minimum IPv4 MTU | Guaranteed to work everywhere |
...
# Test MTU with ping (don't fragment flag)
ping -M do -s 1472 192.168.1.1   # 1472 + 28 (IP+ICMP headers) = 1500
# If packet too big: "Frag needed and DF set"
...
**Common MTU issues:**
- VPN connections dropping → reduce MTU
- Large file transfers failing → MTU mismatch
- SSH hangs after login → MTU path discovery blocked

***

### Default Gateway and Routing

The **default gateway** is the router that forwards traffic to destinations outside your local subnet. Think of it like a building's front door — to reach anything outside your building (subnet), you must go through that door. Without a gateway, you can only communicate with devices on your own network.

**Key terms:**
- **Gateway** — The device (usually a router) that connects your local network to other networks.
- **Next hop** — The immediate next device a packet is sent to on its way to the final destination. In `ip route add ... via 10.0.0.1`, the "via" address is the next hop.
- **Routing table** — A list of rules that tells your machine "to reach network X, send packets via gateway Y."

```mermaid
graph LR
    A[Your Server<br/>192.168.1.100] -->|Same subnet| B[Other Server<br/>192.168.1.50]
    A -->|Different subnet| C[Gateway<br/>192.168.1.1]
    C --> D[Internet / Other Networks]
    
    style C fill:#ff9,stroke:#333
```

```bash
# View current default gateway
...
1. Check if destination is in the same subnet as any local interface → send directly
2. If not, look for a specific route to that network → use that gateway
3. If no specific route, use the default gateway (0.0.0.0/0)

| Route Entry | Meaning |
|-------------|---------|
| `default via 192.168.1.1` | Send unknown destinations to 192.168.1.1 |
| `192.168.1.0/24 dev eth0` | Local subnet – no gateway needed |
| `10.0.0.0/8 via 192.168.1.254` | Use 192.168.1.254 for 10.x.x.x networks |
...
- [1.1.3 CLI Fundamentals](../../1-Linux/Subchapter_1.1/1.1.3_CLI_Fundamentals_and_Bash_Basics.md) – Command-line basics for IP commands
- [1.2.1 FHS](../../1-Linux/Subchapter_1.2/1.2.1_FHS_Filesystem_Hierarchy_Standard.md) – `/proc/sys/net/` for kernel network parameters
- [2.1.1 OSI and TCP/IP Models](./2.1.1_OSI_and_TCP_IP_Models.md) – Network layer concepts for IP addressing
- **Next:** [2.1.3 Subchapter Review](./2.1.3_Subchapter_Review.md)
