...
| `logging.Filter` | Intercepts log records before they reach handlers. Return `True` to keep, `False` to suppress. Use for redacting secrets and suppressing noisy records. |
| `stream=True` in requests | Downloads only headers immediately; body is fetched lazily via `iter_content()` or `iter_lines()`. Required for large files and infinite streams. |
| `iter_lines(decode_unicode=True)` | Iterates response body line-by-line. Use for JSON-lines APIs (Kubernetes watch) and SSE streams. |
...
| `logging.Filter` for secret redaction and record suppression | 9.3.1 |
...
| Streaming responses (`iter_lines`, `iter_content`, `stream=True`) | 9.3.2 |
