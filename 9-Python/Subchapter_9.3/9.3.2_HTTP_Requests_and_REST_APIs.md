...
flowchart TD
    ...
    B -->|"2xx"| C["Success — return data"]
    B -->|"4xx (client error)"| D["Do not retry —<br/>fix the request"]
    B -->|"5xx / 429 / timeout"| E{"attempts < max?"}
    E -->|"no"| F["Give up — raise error"]
    E -->|"yes"| G["sleep = base * 2^attempt<br/>+ random jitter"]
    G --> H["attempt++"]
    H --> A
