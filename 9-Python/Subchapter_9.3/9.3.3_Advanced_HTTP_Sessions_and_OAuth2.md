...
> **Analogy:** Without a Session, every API call is like driving to the store, parking, buying one item, driving home, then driving back for the next item. With a Session (connection pooling), you drive there once, park once, and buy everything in one trip. The "parking spot" (TCP connection) stays reserved for you while you shop.
...
> **⚠️ Advanced (skip on first pass):** This OAuth2 client is a production pattern you'll implement when integrating with services like GitHub Apps or cloud provider APIs. First ensure you're comfortable with basic `requests.get()` with a static Bearer token (9.3.2). Return here when your token starts expiring mid-script.

> **Analogy:** An OAuth2 access token is like a visitor badge at an office building. It expires after an hour. The `OAuth2Client` class is like having a smart assistant who checks your badge before every meeting — if it's about to expire, they quietly visit the front desk (auth server) and get you a new one, so you never get turned away at a door.
...
## Part 6: Kubernetes Python Client — Beyond `kubectl` via subprocess

> **When to use:** If your script only runs `kubectl get pods`, use `subprocess`. But if you need to **watch** resources in real-time, create complex objects programmatically, or build operators/controllers in Python, the official **`kubernetes` Python client** is far more powerful and type-safe than shelling out.
...
### Authentication — Loading kubeconfig
...
# Load from inside a pod (uses ServiceAccount token automatically)
# /var/run/secrets/kubernetes.io/serviceaccount/token
config.load_incluster_config()

# Auto-detect: in-cluster if available, else kubeconfig
try:
    config.load_incluster_config()
...
for pod in pods.items:
    print(f"{pod.metadata.name:40} {pod.status.phase:12} "
          f"Node: {pod.spec.node_name}")

# Get a specific pod
pod = v1.read_namespaced_pod(name='web-abc123', namespace='production')
print(f"IP: {pod.status.pod_ip}")
print(f"Restarts: {pod.status.container_statuses[0].restart_count}")
...
for d in deployments.items:
    print(f"{d.metadata.name:30} "
          f"Ready: {d.status.ready_replicas}/{d.spec.replicas}")
...
apps_v1.patch_namespaced_deployment(
    ...
    body={
        'spec': {
            'template': {
                'spec': {
                    'containers': [{'name': 'app', 'image': 'myapp:v2.0'}]
                }
            }
        }
    }
...
> **Why watch matters:** `kubectl get pods --watch` streams changes in real-time. The Python client `Watch` does the same — essential for building controllers, alert systems, or deployment status trackers.
...
# Stream pod events (runs forever until break)
for event in w.stream(v1.list_namespaced_pod, namespace='production', timeout_seconds=300):
    ...
    if event_type == 'MODIFIED' and phase == 'Failed':
        logger.warning(f"Pod {name} entered Failed state!")
        # Send alert to Slack, PagerDuty, etc.

    if event_type == 'DELETED':
        logger.info(f"Pod {name} was deleted")
...
v1.create_namespaced_config_map(namespace='production', body=config_map)

# Create a Secret
import base64
secret = client.V1Secret(
    metadata=client.V1ObjectMeta(name='app-secrets', namespace='production'),
    type='Opaque',
    data={
        'API_KEY': base64.b64encode(b'my-secret-key').decode(),
    }
)
v1.create_namespaced_secret(namespace='production', body=secret)
```
...
try:
    pod = v1.read_namespaced_pod(name='nonexistent', namespace='default')
...
### `subprocess kubectl` vs `kubernetes` Python Client

| Aspect | `subprocess` + `kubectl` | `kubernetes` Python client |
|--------|--------------------------|----------------------------|
| Setup complexity | Low (just `kubectl` in PATH) | Low (`pip install kubernetes`) |
| Type safety | None (parse JSON strings) | Full (typed objects) |
| Real-time watch | Hard (parse streaming stdout) | Built-in `watch.Watch()` |
| Error handling | Parse stderr strings | Typed `ApiException` |
| Performance | New process per call | Persistent HTTP session |
| IDE support | None | Full auto-complete |
| Best for | Simple scripts, one-off commands | Controllers, operators, complex logic |

---

**End of Subchapter 9.3**

You now have complete HTTP client knowledge: from basic `requests.get()` to async parallel calls, OAuth2 token refresh, pagination, production client design patterns, and the Kubernetes Python client.
