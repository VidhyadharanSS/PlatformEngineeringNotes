...
> **Complexity jump ahead:** Part 1 used `basicConfig()` — one line, good for scripts. Part 2 shows the "real" way used in production services with multiple destinations. The key difference: instead of Python auto-configuring everything, *you* create Handler objects and attach them to loggers manually. Think of it as going from "plug-and-play" to a "custom hi-fi setup — you wire each speaker yourself."
...
### `logging.Filter` — Redacting Secrets and Filtering Records

> **⚠️ Advanced (skip on first pass):** Log filters are a production compliance requirement (SOC2, GDPR). You won't need them until you're building services that handle credentials. Master basic logging (Parts 1–2) first.

> **Why filters matter:** In production, log output inevitably captures sensitive data — API tokens in headers, database passwords in connection strings, PII in user objects. A `logging.Filter` intercepts every log record before it reaches a handler, letting you redact, modify, or suppress records. This is a compliance requirement (SOC2, GDPR) that `print()` cannot satisfy.
...
class SensitiveDataFilter(logging.Filter):
    """Redact sensitive patterns from log messages before they reach handlers"""
    ...
    PATTERNS = [
        ...
        (re.compile(r'AKIA[A-Z0-9]{16}'),
         'AKIA***REDACTED***'),
        (re.compile(r'(?i)(ghp_|gho_|ghu_|ghs_|ghr_)[A-Za-z0-9_]{30,}'),
         '***GITHUB_TOKEN_REDACTED***'),
    ]

    def filter(self, record: logging.LogRecord) -> bool:
        """Modify the log record in-place and return True to keep it"""
        msg = record.getMessage()
        for pattern, replacement in self.PATTERNS:
            msg = pattern.sub(replacement, msg)
        record.msg  = msg
        record.args = ()   # clear args since message is pre-formatted
        return True         # True = keep the record; False = suppress entirely

# Usage
logger = logging.getLogger('myapp')
logger.addFilter(SensitiveDataFilter())

# These log messages will have secrets redacted automatically
logger.info("Connecting with password=s3cr3t123")
# Output: "Connecting with password=***REDACTED***"

logger.debug("Authorization: Bearer ghp_abc123def456...")
# Output: "Authorization: Bearer ***REDACTED***"
```

```python
# Filter that suppresses noisy records entirely
class SuppressHealthCheckFilter(logging.Filter):
    """Suppress /health and /ready endpoint logs (too noisy in Kubernetes)"""
    def filter(self, record: logging.LogRecord) -> bool:
        msg = record.getMessage()
        return '/health' not in msg and '/ready' not in msg
        # False = suppress (don't log); True = keep

# Attach to a specific handler
console_handler = logging.StreamHandler()
console_handler.addFilter(SuppressHealthCheckFilter())
```

> **`filter()` return value:** `True` = keep the record (pass it to the handler). `False` = suppress it entirely. This is the opposite of what many people expect.
