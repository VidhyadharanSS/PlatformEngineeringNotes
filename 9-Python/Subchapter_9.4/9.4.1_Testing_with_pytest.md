...
> **Tip:** The "test pyramid" applies: lots of fast unit tests, fewer integration tests, very few end-to-end tests. If your unit tests need a network, database, or a running cluster, they're integration tests — mark them `@pytest.mark.integration` and skip them by default in CI's fast lane.

> **Warning:** Never test against real production services from CI. Use fixtures (Task 18 in the [Practice Lab](../Subchapter_9.5/9.5.1_Practice_Lab_20_Tasks.md)) or mocks (`unittest.mock.patch`). A flaky test that hits a real API will block deployments and erode trust in the test suite.
...
> **What is a fixture?** In testing, a "fixture" is any pre-configured setup your test needs to run — a temporary file, a database connection, a mock server. In pytest, fixtures are functions decorated with `@pytest.fixture` that *provide* these setups to your test functions automatically. Think of a fixture as a lab technician who prepares your workstation before each experiment and cleans it up afterward.
...
### `autouse=True` — Fixtures That Apply to Every Test

> **Problem:** Some setup must apply to every test in a module — disable network access, set consistent env vars, freeze time, or suppress warnings. Requesting the fixture by name in every test is tedious and error-prone. `autouse=True` makes a fixture apply automatically.

```python
# tests/conftest.py — autouse fixtures activate for EVERY test in the directory
...
def disable_network(monkeypatch):
    """
    Block all outgoing HTTP requests in tests.
    Tests that need network must explicitly mock requests.
    If any test accidentally makes a real HTTP call, it fails immediately
    instead of flaking due to network issues.
    """
    import socket
    ...
    def guard(*args, **kwargs):
        raise RuntimeError(
            ...
            "Use monkeypatch or mocker.patch to mock requests."
        )
...
def clean_env(monkeypatch):
    """Ensure tests don't leak env vars to each other"""
...
def suppress_logging():
    """Prevent log noise during test runs"""
    logging.disable(logging.CRITICAL)
    yield
    logging.disable(logging.NOTSET)   # restore after test
```

```python
# Scoped autouse — apply only to tests in this class
class TestDeployment:
    @pytest.fixture(autouse=True)
    def setup_k8s_mock(self, monkeypatch):
        """Every test in this class gets a mocked kubectl"""
        monkeypatch.setenv('KUBECONFIG', '/dev/null')

    def test_deploy(self):
        # KUBECONFIG is already set — no need to request the fixture
        pass

    def test_rollback(self):
        # KUBECONFIG is already set here too
        pass
```

> **When to use `autouse=True`:**
> - ✅ Disabling network access in all tests
> - ✅ Setting consistent env vars (ENVIRONMENT=test)
> - ✅ Suppressing log noise
> - ✅ Resetting global state between tests
> - ❌ **Don't** use for fixtures that provide a return value (tests can't access the value unless they also request it by name)
