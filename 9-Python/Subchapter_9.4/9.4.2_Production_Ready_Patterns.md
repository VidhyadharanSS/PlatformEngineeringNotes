...
> **Key concept — Decorators (`@something`):**
> You've seen `@pytest.fixture`, `@dataclass`, and now `@retry`. The `@` syntax is called a **decorator**. It wraps a function (or class) with extra behaviour without changing its code.
>
> **Analogy:** A decorator is like gift-wrapping a present. The present (your function) is unchanged inside, but the wrapper adds something — a bow (logging), a tag (type conversion), or a return policy (retry logic). When someone opens it (calls the function), the wrapper can do things before and after the present is revealed.
>
> You don't need to write your own decorators yet — just understand that `@name` means "wrap this function with the `name` function."
...
## Part 9: Python Packaging — `pyproject.toml` and Installable Tools

> **Why packaging matters for platform engineers:** When your Python automation grows beyond a single script into multiple modules (a CLI tool, a shared library, an internal SDK), you need a way to:
> - Install it with `pip install .` or `pip install -e .` (editable mode for development)
> - Distribute it to your team (private PyPI, artifact registry)
> - Define entry points so `myplatform deploy` works from anywhere after install
> - Declare dependencies properly so CI/CD can reproduce the environment

### `pyproject.toml` — The Modern Standard

`pyproject.toml` replaces `setup.py` (legacy) and `setup.cfg` as the single source of truth for Python project metadata.

```toml
# pyproject.toml — minimal production example
[build-system]
...
name = "platform-tools"
version = "1.0.0"
description = "Internal platform engineering CLI tools"
...
platform-deploy = "platform_tools.cli:deploy_main"
platform-check  = "platform_tools.cli:health_main"

[tool.pytest.ini_options]
testpaths = ["tests"]
addopts = "-v --tb=short --cov=platform_tools --cov-fail-under=80"
...
├── README.md
├── src/
│   └── platform_tools/        ← Source package (importable)
│       ├── __init__.py
│       ├── cli.py             ← Entry points (deploy_main, health_main)
│       ├── k8s_client.py
│       ├── retry.py
│       └── config.py
├── tests/
...
pip install -e ".[dev]"
# Now you can:
#   1. Import: from platform_tools.k8s_client import ...
#   2. Run CLI: platform-deploy --env prod --image myapp:v1
#   3. Run tests: pytest
...
python -m build
# Creates:
#   dist/platform_tools-1.0.0-py3-none-any.whl
#   dist/platform_tools-1.0.0.tar.gz

# Install the wheel on another machine
pip install platform_tools-1.0.0-py3-none-any.whl
```

### Entry Points — CLI Commands from Python Functions

```python
# src/platform_tools/cli.py
import argparse
import sys

def deploy_main() -> None:
    """Entry point for `platform-deploy` CLI command"""
    parser = argparse.ArgumentParser(description='Deploy to Kubernetes')
    parser.add_argument('--env', required=True, choices=['dev', 'staging', 'prod'])
    parser.add_argument('--image', required=True)
    args = parser.parse_args()

    # Import heavy modules lazily (faster CLI startup)
    from platform_tools.k8s_client import deploy
    success = deploy(args.env, args.image)
    sys.exit(0 if success else 1)

def health_main() -> None:
    """Entry point for `platform-check` CLI command"""
    from platform_tools.k8s_client import health_check
    ok = health_check()
...
```bash
# Upload to private PyPI (e.g., Artifactory, CodeArtifact, GitLab PyPI)
pip install twine
twine upload --repository-url https://pypi.internal.example.com/simple/ dist/*

# Install from private registry
pip install --index-url https://pypi.internal.example.com/simple/ platform-tools

# Or in requirements.txt
--index-url https://pypi.internal.example.com/simple/
platform-tools==1.0.0
```

### Dockerfile for Packaged Python Tool

```dockerfile
FROM python:3.11-slim

WORKDIR /app
COPY dist/platform_tools-1.0.0-py3-none-any.whl .
RUN pip install --no-cache-dir platform_tools-1.0.0-py3-none-any.whl

# Entry point is the CLI command registered in pyproject.toml
ENTRYPOINT ["platform-deploy"]
```

> **`setup.py` vs `pyproject.toml`:** `setup.py` is the legacy approach (still works but not recommended for new projects). `pyproject.toml` is the PEP 517/518/621 standard since 2022. All modern tools (pip, build, pytest, mypy, ruff) read from `pyproject.toml`.
