...
| | Use `autouse=True` fixtures for global test setup | ☐ |
...
| | Use `atexit` for guaranteed cleanup (PID files, temp dirs) | ☐ |
| | Structure code as a package with `__init__.py` and `__all__` | ☐ |
