...
> **⚠️ Advanced (skip on first pass):** Parallel execution uses Python's `concurrent.futures` module, which involves threading concepts. Master sequential `subprocess.run()` first (9.2.1). Come back here when you have a real CI/CD use case that's too slow running commands one-at-a-time.

> **Bridging context:** So far, all our commands run one at a time — the script waits for each to finish before starting the next. In CI/CD pipelines, this is often too slow. The next section shows how to run multiple commands simultaneously using "threads" — lightweight parallel workers inside one program.
