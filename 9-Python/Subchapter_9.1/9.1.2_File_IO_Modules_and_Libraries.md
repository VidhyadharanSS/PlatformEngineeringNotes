...
## Part 4b: The `tempfile` Module — Safe Temporary Files

> **Why `tempfile` matters:** CI/CD scripts, pytest fixtures, and Docker build processes constantly need temporary files that are automatically cleaned up. Using hardcoded paths like `/tmp/myfile.txt` creates race conditions (two processes using the same name) and leaves garbage behind if the script crashes. `tempfile` solves both problems.

```python
import tempfile
from pathlib import Path

# TemporaryDirectory — auto-cleaned up when context exits
with tempfile.TemporaryDirectory() as tmpdir:
    # tmpdir is a string like '/tmp/tmp5n8o2x3k'
    config_path = Path(tmpdir) / 'config.yaml'
    config_path.write_text('server:\n  port: 8080\n')

    # Use the temp dir for any purpose
    subprocess.run(['helm', 'template', '-f', str(config_path), 'mychart/'],
                   capture_output=True, text=True)
# Directory and ALL contents deleted here, even on exception

# NamedTemporaryFile — auto-deleted when closed
with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=True) as f:
    f.write('apiVersion: v1\nkind: ConfigMap\n')
    f.flush()   # ← IMPORTANT: data may be buffered until flush/close
    print(f"Temp file: {f.name}")  # /tmp/tmpXXXXXX.yaml
    # Pass f.name to other tools (file still exists while context is open)
...
try:
    f.write('{"key": "value"}')
    f.close()  # close but don't delete
    # Use f.name elsewhere — file persists
    process_file(f.name)
finally:
    Path(f.name).unlink(missing_ok=True)  # manual cleanup

# mkstemp / mkdtemp — low-level (returns fd or path, no auto-cleanup)
import os
fd, path = tempfile.mkstemp(suffix='.log')
os.write(fd, b'log data\n')
os.close(fd)
# You must delete 'path' manually
```

### When to Use Which `tempfile` Function

| Function | Auto-cleanup | Use Case |
|----------|-------------|----------|
| `TemporaryDirectory()` | ✅ on exit | Temp workspace for builds, tests |
| `NamedTemporaryFile(delete=True)` | ✅ on close | Temp config files passed to commands |
| `NamedTemporaryFile(delete=False)` | ❌ manual | Temp files that outlive the `with` block |
| `mkstemp()` / `mkdtemp()` | ❌ manual | Low-level (returns fd/path only) |

> **`NamedTemporaryFile` on Linux vs Windows:** On Linux, `f.name` is accessible by other processes even while the file is open. On Windows, the file is locked — you must pass `delete=False`, close it, let others read it, then delete manually. For cross-platform scripts, use `delete=False`.

### Real-World Pattern: Temp Kubeconfig for Testing

```python
import tempfile, subprocess, yaml

def run_with_temp_kubeconfig(kubeconfig_data: dict, cmd: list[str]) -> tuple[bool, str]:
    """Write kubeconfig to temp file, run kubectl, clean up"""
    with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=True) as f:
        yaml.dump(kubeconfig_data, f)
        f.flush()
        env = {**os.environ, 'KUBECONFIG': f.name}
