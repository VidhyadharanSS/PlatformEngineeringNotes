...
**Backlinks:** [9.0.1 — Python Runtime & Mental Model](../Subchapter_9.0/9.0.1_Python_Interpreter_Runtime_and_Mental_Model.md) (the foundation for this note) | [Module 1 — Linux](../../1-Linux/) (file operations, environment variables, processes — all revisited in Python) | [Module 3 — Shell Scripting](../../3-Shell-Scripting/) (Bash scripts you will now rewrite in Python) | [Module 4 — Docker](../../4-Docker/) (Docker CLI called via subprocess later in 9.2.1) | [Module 5 — Kubernetes](../../5-Kubernetes/) (kubectl commands and YAML manifests parsed in Python)

**Next note:** [9.1.2 — File I/O, Modules, and Libraries](./9.1.2_File_IO_Modules_and_Libraries.md)

> **Tip:** If you haven't read [9.0.1](../Subchapter_9.0/9.0.1_Python_Interpreter_Runtime_and_Mental_Model.md) yet, do it first. It explains *why* Python behaves the way it does — variables as labels, LEGB scope, the import system — and everything below will make far more sense.
...
## Part 6b: `match/case` — Structural Pattern Matching (Python 3.10+)

> **Analogy:** `match/case` is like a mail sorting machine. You feed in a letter (the value), and the machine checks it against a series of templates (cases). The first template that fits — by shape, address format, or weight — routes the letter to the correct bin. The `_` wildcard is the "everything else" bin.

> **What is `match/case`?** Python 3.10 added structural pattern matching — the Python equivalent of `switch/case` from other languages, but far more powerful. It can match values, types, data structure shapes, and nested patterns. For platform engineers, it replaces chains of `if/elif` when routing Kubernetes event types, parsing API responses, or dispatching CLI commands.
...
def handle_http_status(code: int) -> str:
    match code:
        ...
        case 201:
            return "Created"
        case 301 | 302:                    # OR pattern — matches either
            return "Redirect"
        case 400:
            return "Bad Request"
        case 401 | 403:
        ...
        case _:                            # _ = wildcard, matches everything
            return f"Unknown ({code})"

print(handle_http_status(503))  # "Server Error (503)"
```

### Matching Data Structures (The Killer Feature)

> **⚠️ Advanced (skip on first pass):** Nested dict destructuring with `match/case` is powerful but complex. Master basic `if/elif` first (Part 6), then return here once you're comfortable with dictionaries and Kubernetes event formats (Module 5).

> **Why this is powerful for DevOps:** Kubernetes API responses are deeply nested dicts. `match/case` can destructure and extract values in one step — no chains of `.get()` calls.

```python
def handle_k8s_event(event: dict) -> None:
    """Route Kubernetes watch events by type and object kind"""
    match event:
        case {"type": "ADDED", "object": {"kind": "Pod", "metadata": {"name": name}}}:
            print(f"New pod created: {name}")

        case {"type": "MODIFIED", "object": {"kind": "Pod",
              "status": {"phase": "Failed"}, "metadata": {"name": name}}}:
            print(f"⚠️  Pod failed: {name}")

        case {"type": "DELETED", "object": {"metadata": {"name": name}}}:
            print(f"Resource deleted: {name}")

        case {"type": event_type, "object": {"kind": kind}}:
            print(f"Other event: {event_type} on {kind}")

        case _:
            print(f"Unrecognized event structure")
...
# Output: ⚠️  Pod failed: web-abc123
```

### Matching Types and Sequences

```python
def process_config_value(value) -> str:
    """Handle different config value types"""
    match value:
        case str(s):                       # matches any string
            return s
        case int(n) | float(n):            # matches numeric types
            return str(n)
        case bool(b):                      # MUST come before int (bool is subclass of int)
            return str(b).lower()
        case [first, *rest]:               # matches a list, captures first + rest
            return f"{first} (+{len(rest)} more)"
        case {"host": h, "port": p}:       # matches dict with these keys
            return f"{h}:{p}"
        case None:
            return "null"
        case _:
...
# Sequence unpacking
def parse_cli_command(args: list[str]) -> None:
    match args:
        case ["deploy", env, image]:
            print(f"Deploying {image} to {env}")
        case ["rollback", deployment]:
            print(f"Rolling back {deployment}")
        case ["status"]:
            print("Showing status")
        case ["status", namespace]:
            print(f"Showing status for {namespace}")
        case [cmd, *_]:
...
| Scenario | Use |
|----------|-----|
| 2-3 simple conditions | `if/elif/else` |
| Many value-based branches (like HTTP codes) | `match/case` |
| Destructuring nested dicts/lists | `match/case` ✅ |
| Type-based routing | `match/case` |
| Simple boolean conditions | `if/elif/else` |
| Need Python < 3.10 compatibility | `if/elif/else` |

> **Compatibility note:** `match/case` requires Python 3.10+. If your scripts must run on Python 3.8/3.9 (common in older container images), use `if/elif` chains instead. Check with `python3 --version`.
