# 9.6.1 Databases for Platform Engineers

**Backlinks:** [5.7.5 — Secrets Management](../../5-Kubernetes/Subchapter_5.7/5.7.5_Secrets_Management_Deep_Dive.md) · [5.8.7 — Observability](../../5-Kubernetes/Subchapter_5.8/5.8.7_Three_Pillars_Metrics_Logs_Traces.md) · [5.5 K8s StatefulSets](../../5-Kubernetes/)

**Next note:** [9.6.2 — Message Queues and Async Patterns](9.6.2_Message_Queues_and_Async_Patterns.md)
...
- **Kafka** — event log (see [9.6.2](9.6.2_Message_Queues_and_Async_Patterns.md)).
...
> **Analogy:** A database connection is like a phone line to a pizza place. The restaurant has 100 phone lines. If each of your 50 delivery apps holds 20 lines open "just in case," you've claimed 1,000 lines — but only 100 exist. A connection pool (PgBouncer) is like a receptionist who answers all incoming calls on a few lines and patches them through to the kitchen only when the order is actually being placed.
...
> **⚠️ Advanced (skip on first pass):** Isolation levels and deadlocks are DBA-territory concepts. As a platform engineer, you mainly need to know: (1) the default (Read Committed) is fine for most apps, and (2) your app should retry on deadlock errors. Come back for the details when you're debugging a concurrency bug.
...
Add these to your Grafana dashboards ([5.8.7](../../5-Kubernetes/Subchapter_5.8/5.8.7_Three_Pillars_Metrics_Logs_Traces.md)):
...
flowchart TD
    ...
    Q6 -->|yes| ASY[Offload to async worker<br/>9.6.2]
```
...
- [ ] Runbook for: DB down, replica lag, connection saturation ([5.8.9](../../5-Kubernetes/Subchapter_5.8/5.8.9_Incident_Response_and_On_Call.md))
...
Next: [9.6.2 — Message Queues and Async Patterns](9.6.2_Message_Queues_and_Async_Patterns.md) — Kafka, RabbitMQ, Redis, DLQs, and when to use which.
