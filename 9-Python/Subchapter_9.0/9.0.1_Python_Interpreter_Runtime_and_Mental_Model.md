...
> **Analogy:** Think of Python's execution like a restaurant. Your `.py` file is the customer's order written on a napkin (human-readable). The **parser** is the waiter who transcribes it onto an order ticket (AST). The **compiler** translates that into kitchen shorthand the cook understands (bytecode). The **VM** is the cook who actually executes the instructions step by step.
...
> **⚠️ Advanced (skip on first pass):** Bytecode disassembly is a diagnostic tool — you don't need it to write Python. Come back here after completing 9.1 if you're curious about what happens under the hood.
...
> **⚠️ Advanced (skip on first pass):** For 99% of platform engineering, you'll only ever use CPython. The table below is for awareness — don't memorize it.
...
### Beginner Dependency Workflow

Use this workflow for almost every platform automation project:

```bash
# 1. Create an isolated environment inside the project
python3 -m venv .venv
source .venv/bin/activate
...
# 3. Install runtime/test libraries
python -m pip install requests pyyaml pytest

# 4. Record exact versions for repeatable installs
python -m pip freeze > requirements.txt

# 5. Recreate the same environment elsewhere
python -m pip install -r requirements.txt
```

**Mental model:** `apt install python3-venv` installs the system ability to create virtual environments. `python -m pip install ...` installs libraries into the active venv. `requirements.txt` is the simple path for scripts; `pyproject.toml` is the cleaner path when you are building a reusable package, CLI, or library.
...
> **Analogy:** LEGB is like asking a question in a large organisation. First you ask your **L**ocal team (your function). If nobody knows, you ask the **E**nclosing department (the outer function). Still nothing? You go to the **G**lobal company directory (module level). Finally, you check the **B**uilt-in phone book that comes with every Python installation (`print`, `len`, etc.).
