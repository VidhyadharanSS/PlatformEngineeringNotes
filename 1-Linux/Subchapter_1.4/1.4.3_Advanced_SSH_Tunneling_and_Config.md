...
Production environments rarely allow direct access to internal services. Databases, message queues, and internal APIs sit behind firewalls, accessible only through **bastion hosts** (jump boxes).

**What is a bastion host?** A bastion (or "jump box") is a single, hardened server that's the *only* machine in your network accessible from the internet via SSH. To reach any other internal server, you first SSH into the bastion, then SSH from there to the target. It's like a security checkpoint — everyone must pass through it, making it easier to audit and protect access.
