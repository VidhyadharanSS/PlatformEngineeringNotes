...
The diagram below shows the SSH connection setup. In step 3, **Diffie-Hellman key exchange** allows both sides to agree on a shared secret key without ever sending it over the network — think of it as two people mixing paint colors publicly but ending up with the same secret color that no observer can reproduce.
