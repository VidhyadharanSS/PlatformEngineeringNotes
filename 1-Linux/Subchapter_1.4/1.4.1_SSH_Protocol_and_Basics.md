...
* File permissions from 1.2.2 (`chmod 600 ~/.ssh/id_rsa`) are critical for SSH key security.

* User management from 1.3.1 (the `alice` user in examples).

* `rsync` from 1.2.3 uses SSH as its transport – understanding SSH basics is prerequisite for remote `rsync`.
