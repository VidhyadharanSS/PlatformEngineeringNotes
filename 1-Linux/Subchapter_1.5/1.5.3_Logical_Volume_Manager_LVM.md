...
Snapshots create a point-in-time copy of an LV without copying all the data initially. Changes are stored in the snapshot, making it space-efficient.
...
# Or for AWS:
sudo sg_scan -i  # Rescan all SCSI devices
...
# Ensure LVM is enabled in initramfs (usually automatic)
...
* Partitioning from 1.5.1 (LVM requires partition type `8e` or whole disk)

* Mounting and fstab from 1.5.2 (LVs are mounted like any other block device)

* Filesystem creation from 1.5.1 (`mkfs.ext4`, `resize2fs`) is used on LVs

* `dd` from 1.1.2 used to create test disk images

* `losetup` (loop devices) allows testing without real disks
