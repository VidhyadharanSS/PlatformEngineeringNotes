...
> **Warning:** Shrinking is risky — if you shrink the logical volume smaller than the filesystem occupying it, you will **permanently destroy data** with no recovery possible. The filesystem must always be shrunk *first*, and you must get the sizes exactly right. Always backup first. XFS **cannot** be shrunk at all — only ext4 supports this.
...
| [1.1.2 Introduction to the Shell and Basic Commands](../Subchapter_1.1/1.1.2_CLI_Basics_and_Philosophy.md) | `dd` is reused here to create raw disk image files for safe storage practice labs. |
