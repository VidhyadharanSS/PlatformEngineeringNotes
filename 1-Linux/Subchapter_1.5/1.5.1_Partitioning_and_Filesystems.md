...
A **partition table** is like a table of contents for a book — it tells the operating system where each section (partition) of the disk starts and ends, so it knows how to read the data. There are two formats in common use:
...
Swap is disk space used as virtual memory when physical RAM is exhausted. Critical for servers under memory pressure.
...
# Method 2: Swap File (modern, flexible – works on ext4/XFS)
...
# Make persistent (add to /etc/fstab – covered in 1.5.2)
...
# Disable ALL swap (needed before resizing)
sudo swapoff -a
...
| System Type              | RAM       | Recommended Swap        | Reasoning                              |
| ------------------------ | --------- | ----------------------- | -------------------------------------- |
| Desktop / Dev            | ≤8 GB     | Equal to RAM (8 GB)     | Allows hibernate                       |
| Server (no hibernate)    | 8-64 GB   | 50% of RAM (4-32 GB)    | Safety net for memory spikes           |
| Database server          | 64+ GB    | 4-8 GB (minimal)        | Databases prefer RAM, swap kills perf  |
| Kubernetes node          | Any       | Disabled (`swapoff -a`)  | K8s requires swap off by default       |
...
# Lower = prefer keeping data in RAM (good for databases)
...
> **Tip:** For Kubernetes nodes, swap must be disabled (`swapoff -a`) and removed from `/etc/fstab`. Kubelet refuses to start if swap is enabled (unless `--fail-swap-on=false`).
