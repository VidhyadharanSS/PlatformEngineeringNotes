...
| Concept                   | Explanation                                                                             |
| ------------------------- | --------------------------------------------------------------------------------------- |
| `losetup`                 | Creates loop devices from regular files for testing storage commands without real disks |
| `sg_scan`                 | Rescans SCSI bus to detect new or resized devices (used in cloud workflows)             |
| `dracut` / `initramfs`    | Initial RAM filesystem that loads LVM modules before root is mounted                    |
| `systemd-fstab-generator` | Converts fstab entries into systemd mount units                                         |
| `pg_start_backup()`       | PostgreSQL function to put database in backup mode before filesystem snapshots          |
| Copy-on-write (COW)       | Snapshot technique where original blocks are copied before modification                 |
