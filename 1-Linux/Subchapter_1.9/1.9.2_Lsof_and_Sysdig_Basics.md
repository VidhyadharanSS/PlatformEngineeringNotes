...
* Process management from 1.6.1 (lsof shows open files per process, integrates with `kill`)

* Filesystem from 1.5.x (recovering deleted files uses `/proc/PID/fd/` and inode knowledge)

* Network from Module 2 (upcoming) – lsof network output will make more sense after learning TCP states

* `watch` command used in lsof example (repeats command every N seconds)
