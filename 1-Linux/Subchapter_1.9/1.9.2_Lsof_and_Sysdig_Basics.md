...
## Strace: Tracing System Calls for a Single Process

While `lsof` shows what files are open, `strace` shows **every system call** a process makes in real time — file operations, network calls, memory allocations, and signals.

### When to Use strace

- **"Why is this process hanging?"** → See which syscall it's stuck on
- **"Why does this program fail silently?"** → See which file/path it can't open
- **"Why is this slow?"** → See excessive syscalls or slow I/O
...
# Shows: openat(), stat(), access()

# Trace only network syscalls
strace -e trace=network curl https://example.com

# Trace with timestamps (find slow calls)
strace -T ls /etc 2>&1 | sort -t'<' -k2 -rn | head -5
# -T shows time spent in each syscall: <0.000123>

# Follow child processes (fork/exec)
strace -f /usr/sbin/nginx

# Write output to file (stderr is default)
strace -o /tmp/trace.log -p 1234

# Count syscall statistics (summary)
strace -c ls /etc
# % time  calls  syscall
# 45.00      12  openat
# 30.00       8  read
...
```bash
# Why does my app fail to start? (find missing files)
strace -e trace=openat ./myapp 2>&1 | grep "ENOENT"
# openat(AT_FDCWD, "/etc/myapp/config.yaml", ...) = -1 ENOENT (No such file)

# Why is this process using 100% CPU?
sudo strace -c -p $(pgrep myapp) 
# Press Ctrl+C after 10 seconds to see summary
# If futex() dominates → lock contention
# If read()/write() dominates → I/O bottleneck

# What config files does nginx read on startup?
strace -e openat /usr/sbin/nginx 2>&1 | grep -v ENOENT
```

> **Warning:** `strace` significantly slows down the traced process (10-100x). Never use on production processes under heavy load — use `perf` or eBPF/`bpftrace` instead for production tracing.
...
**Next note (1.9.3)** is the Subchapter Review for advanced Linux utilities. GPU node operations now live in Kubernetes: [5.8.6 GPU Nodes, NVIDIA Device Plugin, and Operations](../../5-Kubernetes/Subchapter_5.8/5.8.6_GPU_Nodes_NVIDIA_Device_Plugin_and_Operations.md).

---

## Backlinks

- [1.6.1 Process Management](../Subchapter_1.6/1.6.1_Process_Lifecycle_and_Tools.md) – `lsof` shows open files per process, integrates with `kill`
- [1.5.1 Disk Partitioning and Filesystems](../Subchapter_1.5/1.5.1_Partitioning_and_Filesystems.md) – Recovering deleted files uses `/proc/PID/fd/` and inode knowledge
- [1.1.2 Linux Filesystem Hierarchy](../Subchapter_1.2/1.2.1_FHS_Deep_Dive.md) – "Everything is a file" philosophy that `lsof` exploits
