...
* Process management from 1.6.1 (GPU processes appear in `ps` and can be killed with `kill`)

* Systemd from 1.6.2 (`nvidia-persistenced` is a systemd service)

* File permissions from 1.2.2 (`nvidia-smi` requires appropriate permissions, often `sudo`)

* Package management from 1.7.x (driver installation uses `apt` or `dnf`)
