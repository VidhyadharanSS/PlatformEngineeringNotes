...
| Concept           | Explanation                                                                                       |
| ----------------- | ------------------------------------------------------------------------------------------------- |
| `rpm2cpio`        | Converts RPM package to cpio archive for file extraction. Used in Q1 to extract individual files. |
| `ldconfig`        | Configures dynamic linker cache (`/etc/ld.so.cache`). Used in Q4 to add custom library paths.     |
| `LD_LIBRARY_PATH` | Environment variable to override library search paths. Used in Q4 for custom OpenSSL.             |
| `rpath`           | Hardcoded library search path in executable. Alternative to `LD_LIBRARY_PATH`.                    |
| `deadsnakes` PPA  | Third-party repository for newer Python versions on Ubuntu. Mentioned in Q2.                      |
| `dnf builddep`    | Installs build dependencies for a package from repository. RHEL equivalent of `apt build-dep`.    |
| `make altinstall` | Python-specific install target that avoids overwriting system `python3` symlink.                  |

***

**End of Subchapter 1.7 Review**

**Next:** Proceed to Subchapter 1.8 – Text Processing Utilities and Editors (`find`, `grep`, `sed`, `awk`, `vim`, `nano`).
