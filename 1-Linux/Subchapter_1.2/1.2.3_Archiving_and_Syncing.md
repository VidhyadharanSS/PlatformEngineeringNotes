...
`pv` (pipe viewer) shows progress for piped operations. Remember from 1.1.2 that pipes (`|`) connect the output of one command to the input of the next — `pv` sits in the middle and displays how much data is flowing through.

> **Note on `tar -cvf -`:** In these examples, the `-` (dash) means "write to standard output instead of a file." This lets you pipe tar's output into another program (like `pv`, `gzip`, or `split`) instead of saving it to disk first. It's a common Unix pattern for chaining tools together.
...
### Rsync Daemon Mode (Advanced — Production Use)

> **For beginners:** You can skip this section on first reading. Rsync daemon mode is used in large-scale production environments. Come back after you're comfortable with basic rsync over SSH.
