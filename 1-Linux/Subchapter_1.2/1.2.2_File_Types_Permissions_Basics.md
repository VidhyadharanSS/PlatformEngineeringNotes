...
# Why capital X? Without it, chmod -R u+x would make ALL files executable (including text files).
# Capital X intelligently adds execute only to directories (for traversal) and files that are already executable.
```
...
**Why these specific numbers?** Each position represents a power of 2 — this means any combination of r, w, and x produces a unique sum (0–7). Think of it as three on/off switches: read is the "4 switch," write is the "2 switch," execute is the "1 switch." Flip the ones you want, add them up.
