...
Think of standard Linux permissions as having exactly **3 keys** to a room: one for the owner, one for a specific group, and one generic key for everyone else. ACLs let you create **individual keys for specific people** — Alice gets read/write, Bob gets read-only, Charlie gets nothing, all on the same file.
...
## Part 5: Linux Capabilities – The Modern Alternative to SUID (Advanced)

> **For beginners:** Capabilities are an advanced security feature you'll encounter when hardening production systems or working with containers. If you're still getting comfortable with basic permissions and SUID/SGID, skip this section for now and come back after completing Module 1.6 (Processes & Services).
