...
### Survival Kit (Learn These 5 Things First)

If you remember nothing else about Vim, memorize these — they will get you through any emergency edit on a remote server:

| Action | Keys | What it does |
|--------|------|-------------|
| Start typing | `i` | Enter Insert mode (now you can type normally) |
| Stop typing | `Esc` | Return to Normal mode (stop inserting text) |
| Save and quit | `:wq` then Enter | Write the file and exit |
| Quit without saving | `:q!` then Enter | Discard changes and exit |
| Search for text | `/` then type your search, then Enter | Find text in the file (`n` for next match) |

**The workflow:** Open file → press `i` to type → make your edit → press `Esc` → type `:wq` to save. That's it. Everything below expands on this foundation.
...
- [1.2.2_Permissions_and_Ownership.md](../Subchapter_1.2/1.2.2_File_Types_Permissions_Basics.md) – Editors respect file permissions; read-only files require `:w!` or `sudo`
