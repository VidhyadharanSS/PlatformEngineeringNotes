...
| Concept                  | Explanation                                                                                                 |
| ------------------------ | ----------------------------------------------------------------------------------------------------------- |
| `xargs`                  | Builds and executes command lines from standard input. Used with `find -print0` for safe filename handling. |
| `sort -rn`               | Sorts numerically in reverse order (highest first). Used in Q4 for top IP addresses.                        |
| `uniq -c`                | Counts duplicate adjacent lines. Used with `sort` to count occurrences.                                     |
| `gsub()`                 | Awk function for global substitution. Used in Q4 to remove brackets from timestamp.                         |
| `match()` and `substr()` | Awk functions for extracting substrings based on regex patterns.                                            |
| `split()`                | Awk function to split string into array. Used to parse URLs and timestamps.                                 |
| Busybox vi               | Minimal vi implementation on embedded systems. Has limited features compared to full vim.                   |
| `head -n -5`             | Output all but last 5 lines. Used in Q5 as alternative to `vi` for deleting lines.                          |

***

**End of Subchapter 1.8 Review**

**Next:** Proceed to Subchapter 1.9 – Advanced Utilities and Specialized Tools (`screen`, `tmux`, `lsof`, Nvidia GPU Management).
