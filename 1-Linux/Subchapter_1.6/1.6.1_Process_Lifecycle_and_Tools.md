...
> **Analogy:** A process is like a worker in a factory. Each worker (process) has a badge number (PID), a supervisor who hired them (PPID), and a clearance level (UID/GID) that determines which rooms they can access.
...
**Fix:** Kill the parent process. This is safe because the zombie itself has *already finished running* — it's not doing any work. When you kill the parent, the zombie gets re-parented to PID 1 (systemd), which automatically reaps it (reads its exit status and removes it from the process table).

```bash
# Find parent of zombie (assuming PID 1234 is zombie)
ps -o ppid= -p 1234
# Kill parent (only do this if the parent is a buggy daemon, not a critical system process)
