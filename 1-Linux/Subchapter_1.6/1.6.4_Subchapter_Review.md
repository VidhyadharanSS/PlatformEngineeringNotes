...
| `kill`, `killall`, `pkill` commands                                  | 1.6.1         |
...
| Concept                  | Explanation                                                                                                      |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `flock`                  | File lock utility to prevent overlapping cron jobs. Used in Q3 to ensure backup script doesn't run concurrently. |
| `strace`                 | Traces system calls of a process. Mentioned in Q1 for debugging high-CPU processes.                              |
| `wait()` / `waitpid()`   | System calls that parent processes use to collect child exit status. Missing calls cause zombies.                |
| `SIGCHLD`                | Signal sent to parent when child terminates. Can trigger reaping of zombies.                                     |
| `systemd-analyze verify` | Validates systemd unit file syntax without loading it.                                                           |
| `systemctl show`         | Displays all properties of a systemd unit (including environment, limits).                                       |
| `pg_dumpall`             | PostgreSQL utility for backing up all databases. Used in Q3 example.                                             |
| `set -euo pipefail`      | Bash strict mode – exit on error, unset variable, pipefail. Used in robust scripts.                              |
