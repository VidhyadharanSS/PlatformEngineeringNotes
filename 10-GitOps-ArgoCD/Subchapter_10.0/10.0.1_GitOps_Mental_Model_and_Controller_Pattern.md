...
> **Analogy — the thermostat:** A home thermostat works the same way. You set the desired temperature (21°C). The thermostat continuously reads the actual temperature. If the room is too cold, it turns on the heater. If too hot, it turns it off. It never stops checking — it *reconciles* forever. The Kubernetes controller is a thermostat for your infrastructure.
...
> **Jargon buster:**
> - **SOPS** — "Secrets OPerationS", a Mozilla-created CLI tool that encrypts specific fields inside YAML files (not the whole file).
> - **age** — a modern, simple encryption tool (alternative to GPG). Generates key pairs for encrypting/decrypting.
> - **KMS** — "Key Management Service", a cloud provider service (AWS KMS, GCP KMS) that manages encryption keys for you.
