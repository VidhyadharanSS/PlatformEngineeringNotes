# 10.4.1 Cloud Primitives and IaC

**Backlinks:** [2.2 Networking Basics](../../2-Networking/) · [8.4.2 — Supply Chain Security](../../8-CICD/Subchapter_8.4/8.4.2_Supply_Chain_Security_and_OWASP.md) · [10.1 GitOps Principles](../../10-GitOps-ArgoCD/Subchapter_10.1/)

**Next note:** [2.5.2 — DNS, TLS, and Certificates Deep Dive](../../2-Networking/Subchapter_2.5/2.5.2_DNS_TLS_and_Certificates_Deep_Dive.md)
...
> **Analogy:** A VPC is like a private office building. The **public subnets** are the lobby and reception — anyone from the street can walk in. The **private subnets** are the office floors — you need a badge (NAT gateway) to go out, and outsiders can't get in directly. The **data subnets** are the vault in the basement — even employees need special clearance.
...
> **⚠️ Advanced (skip on first pass):** IRSA requires understanding both K8s ServiceAccounts (Module 5.8) and AWS IAM roles. On first read, just know that IRSA exists and means "no AWS keys stored anywhere." Come back when you're setting up EKS workloads.
...
> **Analogy:** Terraform state is like a blueprint that says "wall A is here, door B is there." If you lose the blueprint, Terraform doesn't know what already exists in the building — it might try to build a second wall A right next to the first one. If two people edit the blueprint at the same time without coordinating, they'll end up with conflicting plans. That's why state is stored remotely with locking.
...
Next: [2.5.2 — DNS, TLS, and Certificates Deep Dive](../../2-Networking/Subchapter_2.5/2.5.2_DNS_TLS_and_Certificates_Deep_Dive.md) — ACME, cert-manager, SNI, mTLS setup.
...
> **Ready Solution:**
>
> For **10.4.1 Cloud Primitives and IaC**, your recap should identify the core concept, the safest default, the main operational risk, the command or configuration you would verify first, and the signal that proves the system is healthy. A strong answer includes both the technical check and the reason behind it.
