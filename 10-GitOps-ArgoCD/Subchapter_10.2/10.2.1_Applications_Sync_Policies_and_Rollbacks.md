...
The Application CRD (Custom Resource Definition — a way to teach Kubernetes new object types, covered in Module 5.9) is the heart of ArgoCD. It defines:
...
> **What's a finalizer?** A finalizer is a Kubernetes mechanism that says "before deleting this object, run some cleanup first." ArgoCD's finalizer ensures that when you delete an Application, it also deletes the Deployment, Service, etc. that the Application created. Without it, deleting the Application leaves orphaned resources running in your cluster — like tearing up a shopping list while the groceries are still in the cart.
...
> **Analogy — building a house:** You can't install the roof before the walls, or the walls before the foundation. Sync waves are ArgoCD's way of saying "build in this order." Wave -1 is the foundation (namespaces, config), wave 0 is the walls (services, secrets), wave 1 is the roof (deployments that depend on everything else). Lower numbers go first.
...
> **⚠️ Advanced (skip on first pass):** The `ignoreDifferences` feature solves a real production problem (ArgoCD fighting with other controllers), but you won't encounter it until you add autoscaling or service meshes. Skip this section on your first read and return when you see ArgoCD stuck in "OutOfSync" for a resource you didn't touch.

> **Bridging context:** In production, ArgoCD is not the only thing modifying your cluster. The Horizontal Pod Autoscaler (HPA) adjusts replica counts based on CPU load. Istio's sidecar injector adds containers to your pods. These changes are *expected* but ArgoCD sees them as "drift from Git" and tries to undo them. `ignoreDifferences` tells ArgoCD "don't fight over these specific fields."
