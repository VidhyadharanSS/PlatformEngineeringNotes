...
> **Analogy — a franchise manual:** Imagine you're opening a restaurant chain. Instead of individually setting up each restaurant, you create a *franchise manual* (the root Application) that says "open these 10 restaurants with these specifications." Each restaurant is a child Application. Adding a new city? Add one page to the manual and the franchise system handles the rest.
...
## Part 7: ApplicationSet Generators — Advanced Patterns

> **⚠️ Advanced (skip on first pass):** The generators below (merge, matrix, cluster auto-discovery) are production-scale patterns for teams managing 10+ clusters or 50+ applications. For your first GitOps setup, the `list` generator from Part 6 is sufficient. Come back here when you need to deploy the same app to every cluster automatically.

> **Bridging paragraph:** So far you've seen a `list` generator — you manually write out each environment. But what if you have 20 clusters? Typing 20 list entries is tedious and error-prone. The advanced generators below *discover* clusters or directories automatically, so adding a new cluster or app folder in Git is enough — the ApplicationSet generates the Application for you.

### Merge Generator (Combine Multiple Sources)

The `merge` generator combines outputs from multiple generators, merging on a key. This solves the problem of "deploy different apps to different clusters based on labels."

```yaml
# applicationset-merge.yaml
apiVersion: argoproj.io/v1alpha1
kind: ApplicationSet
metadata:
  name: cluster-addons
...
spec:
  ...
  - merge:
      mergeKeys:
      - nameNormalized           # Merge on cluster name
      generators:
      # Base: all clusters
      - clusters:
          selector:
          ...
          values:
            monitoring: "true"   # Default: install monitoring everywhere
            logging: "true"      # Default: install logging everywhere
      # Override: production clusters get extra addons
      - clusters:
          selector:
            matchLabels:
              environment: production
          values:
            security-scanning: "true"  # Only prod gets security scanning
            monitoring-retention: "90d"
      # Override: EU clusters need GDPR addon
      - clusters:
          selector:
            matchLabels:
              region: eu
...
```yaml
# Automatically create apps for all registered clusters
spec:
  generators:
  - clusters:
      selector:
        matchLabels:
          environment: production
      # {{name}}, {{server}}, {{metadata.labels.region}} available in template
```

### Matrix Generator (Cartesian Product)

```yaml
# Deploy every app to every cluster (apps × clusters)
spec:
  generators:
  - matrix:
      generators:
...
|-----------|----------|---------|
| **list** | Known set of values | 3 specific clusters |
| **git** | Directories or files in repo | Each subdirectory = one app |
| **clusters** | Registered ArgoCD clusters | Deploy to all prod clusters |
| **matrix** | Cartesian product of two generators | All apps × all clusters |
| **merge** | Combine + override generators | Base config + cluster-specific overrides |
| **pullRequest** | PR-based preview environments | Deploy branch preview per PR |

---

## Part 8: Secrets Management in GitOps

The biggest challenge in GitOps: **you can't store Kubernetes Secrets in Git.** Secrets are base64-encoded (not encrypted) — anyone with repo access can decode them.
...
# my-password          ← Anyone can read this
```

### Solution 1: Sealed Secrets (Bitnami)

Encrypts secrets so they can be safely stored in Git. Only the cluster can decrypt them.
...
spec:
  encryptedData:
    password: AgBy3i4OJSWK+PiTySYZZA9r... # Encrypted — only cluster can decrypt
```

### Solution 2: External Secrets Operator (ESO)

Syncs secrets from external providers (AWS Secrets Manager, Vault, GCP Secret Manager) into Kubernetes.

```yaml
# Install ESO
# helm install external-secrets external-secrets/external-secrets -n external-secrets

# SecretStore: configure provider connection
...
---
# ExternalSecret: sync from AWS → Kubernetes
apiVersion: external-secrets.io/v1beta1
kind: ExternalSecret
metadata:
  name: db-credentials
...
Encrypts YAML values in-place. ArgoCD has native SOPS support via plugins.

```yaml
# secret.enc.yaml (encrypted values, safe for Git)
apiVersion: v1
kind: Secret
metadata:
  name: db-creds
...
sops:
  kms:
  - arn: arn:aws:kms:us-east-1:123456789:key/xxx
  version: 3.8.1
```
...
# Decrypt (only authorized users/roles)
sops --decrypt secret.enc.yaml
```

### Comparison Table

| Solution | Secrets In Git? | External Provider | Rotation | Complexity | Best For |
|----------|----------------|-------------------|----------|------------|----------|
| **Sealed Secrets** | ✅ Encrypted | No | Manual re-seal | Low | Simple setups, no cloud provider |
| **External Secrets (ESO)** | ❌ References only | AWS/GCP/Vault/Azure | Automatic | Medium | Cloud-native, auto-rotation |
| **SOPS** | ✅ Encrypted | KMS for key management | Manual re-encrypt | Medium | Teams wanting encrypted Git |
| **Vault CSI** | ❌ Injected at runtime | HashiCorp Vault | Automatic | High | Enterprise, Vault-first orgs |

### ArgoCD + External Secrets Operator (Recommended Pattern)

```mermaid
graph LR
    GIT[Git Repo] -->|ExternalSecret YAML| ARGO[ArgoCD]
    ARGO -->|creates ExternalSecret| ESO[External Secrets Operator]
    ESO -->|reads| AWS[AWS Secrets Manager]
    ESO -->|creates| SECRET[K8s Secret]
    SECRET -->|mounts| POD[Application Pod]
    
    style GIT fill:#9f9,stroke:#333
    style AWS fill:#f90,color:#fff
```

> **The #1 GitOps mistake:** Committing base64 Secrets to Git. Even private repos get cloned by CI runners, forked by team members, and backed up to insecure locations. **Always use one of the above solutions.**

---

## Part 9: Complete Production Setup
