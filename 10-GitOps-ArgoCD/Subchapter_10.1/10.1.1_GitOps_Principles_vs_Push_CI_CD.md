...
**Backlinks:** [10.0.1 — Mental Model](../Subchapter_10.0/10.0.1_GitOps_Mental_Model_and_Controller_Pattern.md) (controller pattern, desired vs actual) · [Module 6 — Git](../../6-Git/) · [Module 8 — CI/CD](../../8-CICD/)

> **Tip:** If you haven't read [10.0.1](../Subchapter_10.0/10.0.1_GitOps_Mental_Model_and_Controller_Pattern.md) yet, do it first. The controller-pattern foundation makes every concept below click immediately.
...
> **Important:** GitOps does **not** replace CI — it replaces the **deployment** (CD) half. You still need CI to build, test, and push images. GitOps only changes **how** those images and manifests reach the cluster: agent-in-cluster pulls from Git instead of CI pushing to the cluster.

> **Warning:** A common GitOps anti-pattern is writing image tags like `:latest` or `:main` into Git. Because the tag never changes, ArgoCD sees no diff and won't redeploy. Always commit **immutable tags** (SHA digests, semver, commit SHAs) — this is the whole point of declarative, versioned state.
...
## Part 6: Git Repository Strategy — Monorepo vs Polyrepo

The first architectural decision in GitOps: where do Kubernetes manifests live?

### Option 1: Monorepo (App Code + Manifests Together)
...
### Option 2: Polyrepo (Separate Config Repository)

```
# Repo 1: myapp (source code)
myapp/
├── src/
│   └── main.py
├── Dockerfile
...
# Repo 2: myapp-config (Kubernetes manifests)
myapp-config/
├── base/
│   ├── deployment.yaml
│   └── service.yaml
...
| Aspect | Monorepo | Polyrepo (Recommended) |
|--------|----------|------------------------|
| **CI triggers** | Every code push triggers ArgoCD (noisy) | Only manifest changes trigger ArgoCD |
| **Access control** | Developers see infra configs | Separate RBAC per repo |
| **Blast radius** | Bad commit affects code AND deploy | Isolated failures |
| **Rollback** | Reverting code may revert config | Independent rollbacks |
| **Audit trail** | Mixed commits (code + infra) | Clean infra-only history |
| **ArgoCD recommendation** | Not recommended | **Officially recommended** |

### Why ArgoCD Recommends Polyrepo

```mermaid
graph LR
    subgraph "Source Repo"
        CODE[App Code] --> CI[CI Pipeline]
        CI --> IMG[Build Image]
        IMG --> UPDATE["Update config repo<br/>(new image tag)"]
    end
    
    subgraph "Config Repo (separate)"
        UPDATE --> CONFIG[K8s Manifests]
        CONFIG --> ARGO[ArgoCD]
    end
...
**Key reasons:**
1. **Avoiding infinite loops** — CI builds image → updates manifest → triggers CI again. Polyrepo breaks this cycle
2. **Separation of concerns** — App developers change code; platform engineers change infrastructure
3. **Git history clarity** — `git log` on config repo shows only deployment changes, not code changes
4. **Multiple apps, one config repo** — Many microservices can share one config repo with directory-per-app

### When Monorepo Is Acceptable

| Scenario | Why It Works |
|----------|-------------|
| Solo developer / tiny team | Simplicity outweighs separation |
| Learning / demos | Fewer repos to manage |
| `path:` filtering in ArgoCD | ArgoCD only watches `k8s/` directory |

```yaml
# ArgoCD can filter paths even in monorepo
spec:
  source:
    repoURL: https://github.com/myorg/myapp.git
    path: k8s/overlays/prod  # Only watch this directory
```

> **Interview tip:** When asked "how do you structure Git repos for GitOps?", always mention polyrepo as the production standard and explain why — it's the single most common GitOps architecture question.

---

## Part 7: GitOps Tools Landscape
