...
> **Jargon:**
> - **gRPC** — a high-performance communication protocol (like REST but binary and faster). ArgoCD uses gRPC for its CLI-to-server communication and REST for the web UI. You don't need to understand gRPC internals — just know it's how the CLI talks to the API server efficiently.
> - **OIDC** (OpenID Connect) — a standard protocol for "Login with Google/GitHub/etc." **Dex** is a small server bundled with ArgoCD that speaks OIDC so your team can log in with their existing company accounts instead of sharing one admin password. You won't need Dex until you have multiple team members — ignore it for solo learning.
...
### ArgoCD Notifications Controller (Built-in Since v2.3)

> **⚠️ Advanced (skip on first pass):** Notifications are essential for production but not needed to learn ArgoCD basics. Come back to this section after you've deployed your first application in 10.2.1. For now, just know that ArgoCD can alert you via Slack/email when syncs fail.

The `argocd-notifications-controller` pod appears in every ArgoCD installation but is often overlooked. It sends alerts when sync status or health changes.

```mermaid
graph LR
    CTRL[Application Controller] -->|status change| NOTIFY[Notifications Controller]
    NOTIFY --> SLACK[Slack]
    NOTIFY --> PD[PagerDuty]
    NOTIFY --> EMAIL[Email]
    NOTIFY --> WEBHOOK[Webhooks]
    NOTIFY --> TEAMS[MS Teams]
```

**Configuring Notifications (argocd-notifications-cm):**
    ...
    slack:
      attachments: |
        [{
          ...
          "text": "Application {{.app.metadata.name}} is unhealthy."
        }]
```

**Subscribing an Application to notifications:**
...
|---------|---------------|----------|
| `on-sync-failed` | Sync operation fails | Alert on-call engineer |
| `on-sync-succeeded` | Sync completes successfully | Deployment tracking |
| `on-health-degraded` | App health becomes Degraded | Incident response |
| `on-sync-status-unknown` | Sync status can't be determined | Infrastructure issues |
| `on-deployed` | New revision deployed | Release tracking |

> **Without notifications, GitOps fails silently.** ArgoCD syncs every 3 minutes — if sync fails at 2 AM, nobody knows until morning. Notifications are not optional in production.
