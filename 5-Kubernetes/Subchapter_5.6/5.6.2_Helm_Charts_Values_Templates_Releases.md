# 5.6.2 Helm Charts, Values, Templates, and Releases: The Package Manager for Kubernetes
...
This note covers Helm fundamentals. Note 5.6.1 covered Kustomize; note 5.6.3 is the subchapter review.

**Backlinks:** [5.2.2 - Deployments](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (templates generate YAML) | [5.5.1 - ConfigMaps](../Subchapter_5.5/5.5.1_ConfigMaps_and_Secrets.md)

***

## Part 1: Helm Architecture

A Helm chart is like a recipe book: `Chart.yaml` is the cover (name, version), `values.yaml` is the ingredient list (configurable defaults), and `templates/` are the cooking instructions (YAML with `{{ .Values.X }}` placeholders that get filled in).

> **Note on Tiller:** The diagram below shows Tiller (Helm v2 only). **Helm v3 removed Tiller entirely** — the CLI now talks directly to the Kubernetes API. If you're starting fresh, ignore Tiller references.
>
> **Template basics:** Helm templates use Go's `{{ }}` syntax. `{{ .Values.replicaCount }}` is a placeholder filled from values.yaml. `{{ include "myapp.name" . }}` calls a reusable helper. `| nindent 4` adds proper YAML indentation.
>
> **Why `trunc 63`?** Kubernetes resource names are limited to 63 characters (DNS label limit), so Helm truncates generated names to ensure validity.
...
**Next note (5.6.3)** will be the Subchapter Review for Kustomize and Helm, including a cheatsheet and scenario-based interview questions.

**Backlinks:** [5.2.2 - Deployments](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) | [5.5.1 - ConfigMaps](../Subchapter_5.5/5.5.1_ConfigMaps_and_Secrets.md) | [5.5.2 - Autoscaling](../Subchapter_5.5/5.5.2_Autoscaling_HPA_VPA_Cluster_Autoscaler.md)
