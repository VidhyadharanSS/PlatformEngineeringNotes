# 5.5.1 ConfigMaps and Secrets: Managing Configuration and Sensitive Data
...
> **⚠️ Base64 is NOT encryption** — it's just encoding (like writing in a different alphabet). Anyone who can access the Secret can decode it instantly with `echo <data> | base64 -d`. For real security, enable encryption at rest, restrict RBAC access, or use an external secrets manager.

This note covers ConfigMaps and Secrets. Note 5.5.2 covers autoscaling; note 5.5.3 is the subchapter review.

**Backlinks:** [5.2.1 - Workloads](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (Pods) | [Module 1 - Env Vars](../../1-Linux/Subchapter_1.3/1.3.2_Shell_Profiles_and_Environment.md) | [5.4.1 - Volumes](../Subchapter_5.4/5.4.1_Ephemeral_Volumes_emptydir_hostPath.md)
...
The `stringData` field lets you write secrets in plain text — Kubernetes automatically base64-encodes them. This is the recommended way to write Secret YAMLs.
...
metadata:
  name: db-secret
type: Opaque
stringData:                          # Plain text — K8s encodes for you
  username: admin
  password: S3cr3tPass!
  connection-string: "postgresql://admin:S3cr3tPass!@postgres:5432/mydb"
```

```bash
# Apply (K8s stores as base64 internally)
...
| Field | Input Format | When to Use |
|-------|-------------|-------------|
| `data` | Base64-encoded | Programmatic generation, CI/CD pipelines |
| `stringData` | Plain text | Hand-written YAMLs, readability |

**Note:** If both `data` and `stringData` contain the same key, `stringData` takes precedence. After applying, Kubernetes stores everything in `data` (base64). The `stringData` field is write-only — it never appears in `kubectl get secret -o yaml` output.
...
**Next note (5.5.2)** will cover **Autoscaling: HPA, VPA, and Cluster Autoscaler** – horizontal pod autoscaling, vertical pod autoscaling, and node autoscaling.

**Backlinks:** [Module 1 - Env Vars](../../1-Linux/Subchapter_1.3/1.3.2_Shell_Profiles_and_Environment.md) | [5.4.1 - Volumes](../Subchapter_5.4/5.4.1_Ephemeral_Volumes_emptydir_hostPath.md) | [5.2.1 - Pods](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md)
