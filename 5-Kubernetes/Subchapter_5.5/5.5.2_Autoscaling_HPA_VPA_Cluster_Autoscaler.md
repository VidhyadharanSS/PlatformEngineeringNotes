# 5.5.2 Autoscaling: HPA, VPA, and Cluster Autoscaler – Right-Sizing Your Cluster
...
This note covers all three. Note 5.5.1 covered ConfigMaps/Secrets; note 5.5.3 is the subchapter review.

**Backlinks:** [5.2.2 - Deployments](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (HPA scales replicas) | [Module 4 - Resources](../../4-Docker/Subchapter_4.3/4.3.1_Container_Lifecycle_and_Resource_Management.md) (requests/limits) | [5.1.2 - metrics-server](../Subchapter_5.1/5.1.2_Cluster_Setup_kubeadm_Kind_Multi_Node.md)
...
> **Tip — start simple:** Begin with CPU-based HPA (below). Custom metrics with Prometheus adapters are marked as advanced further down.
>
> **What is the stabilization window?** It prevents flapping — without it, HPA could scale down immediately after a traffic spike ends, then scale up again seconds later. The default 5-minute window waits to confirm the lower load is sustained.

### HPA Control Loop

```mermaid
flowchart LR
    A["metrics-server"] -->|"pod CPU / memory"| B["HPA controller"]
    B -->|"every 15s"| C{"current vs target?"}
    C -->|"above target"| D["scale up"]
    C -->|"below target"| E["scale down"]
    C -->|"within band"| F["no change"]
    D --> G["update replicas on Deployment"]
    E --> G
    G --> H["ReplicaSet creates / removes pods"]
    H --> A
...
## Part 7: KEDA – Kubernetes Event-Driven Autoscaling

> **⚠️ Advanced (skip on first pass):** KEDA is an extension for event-driven workloads (queue consumers, cron jobs). Master HPA (Parts 1–3) first — come back here when you need to scale based on queue depth or scale to zero.

KEDA extends HPA with event-driven scaling, including **scale-to-zero** capability. It supports 60+ event sources beyond CPU/memory.

### Why KEDA Matters

| Problem with standard HPA | KEDA Solution |
|---|---|
| Only CPU/memory metrics natively | Scale on queue length, HTTP requests, cron schedules, cloud events |
| Minimum 1 replica | Scale to zero (save costs for idle workloads) |
| Complex custom metrics setup | Simple ScaledObject YAML |

### KEDA Architecture

```mermaid
graph TD
    A[KEDA Operator] -->|monitors| B[Event Sources<br/>Kafka, RabbitMQ, SQS, Prometheus, Cron]
    A -->|creates/manages| C[HPA]
    C -->|scales| D[Deployment/Job]
    A -->|scale to zero| D
...
### ScaledObject – Scale on Queue Length
...
### ScaledObject – Scale on Prometheus Metric
...
### ScaledJob – Scale Jobs (Batch Processing)
...
| Trigger | Use Case |
|---------|----------|
| `rabbitmq` | Message queue consumers |
| `kafka` | Kafka topic lag |
...
| `http` | HTTP request rate (with KEDA HTTP addon) |
...
# Describe (see trigger status)
kubectl describe scaledobject rabbitmq-consumer

# Check HPA created by KEDA
kubectl get hpa
# NAME                          REFERENCE                     TARGETS       MINPODS   MAXPODS
# keda-hpa-rabbitmq-consumer    Deployment/consumer           5/5 (avg)     0         30
```

***

**Next note (5.5.3)** will be the Subchapter Review for Configuration, Secrets, and Autoscaling, including a cheatsheet and scenario-based interview questions.

**Backlinks:** [5.1.2 - metrics-server](../Subchapter_5.1/5.1.2_Cluster_Setup_kubeadm_Kind_Multi_Node.md) | [5.2.2 - Deployments](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) | [Module 4 - Resources](../../4-Docker/Subchapter_4.3/4.3.1_Container_Lifecycle_and_Resource_Management.md)
