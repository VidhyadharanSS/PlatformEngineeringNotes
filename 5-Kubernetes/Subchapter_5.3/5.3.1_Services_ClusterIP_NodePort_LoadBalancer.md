# 5.3.1 Services: ClusterIP, NodePort, and LoadBalancer – Exposing Pods to the Network
...
This note covers Service types. Note 5.3.2 covers Ingress and Gateway API; note 5.3.3 covers Network Policies; note 5.3.4 covers DNS/CoreDNS; note 5.3.5 is the subchapter review.

**Backlinks:** [5.2.1 - Workloads](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (Pod labels/selectors) | [5.3.4 - CoreDNS](5.3.4_DNS_Basics_and_CoreDNS.md) (service DNS) | [Module 2 - DNS](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md) (DNS fundamentals) | [Module 2 - iptables](../../2-Networking/Subchapter_2.3/2.3.2_Firewalling_with_iptables_nftables.md) (kube-proxy)

***

## Part 1: Service Overview

A Service is like a company phone number that stays the same even when employees (pods) change. Calls are routed to whoever is currently working. Without Services, you'd have to track every pod's constantly-changing IP address.
...
## Part 5: externalTrafficPolicy – Preserving Client Source IP

By default, kube-proxy rewrites the source IP when routing external traffic. This loses the real client IP address. `externalTrafficPolicy` controls this behavior for NodePort and LoadBalancer services.

### Cluster vs Local

| Policy | Source IP | Load Balancing | Extra Hops |
|--------|-----------|----------------|------------|
| **Cluster** (default) | Replaced with node IP | Even across all pods | Yes (inter-node) |
| **Local** | Preserved (real client IP) | Only to pods on receiving node | No |
...
### How `Cluster` (Default) Works

```mermaid
graph LR
    C[Client<br/>203.0.113.5] -->|NodePort| N1[Node-1]
    N1 -->|SNAT to Node-1 IP| N2[Node-2]
    N2 --> P[Pod on Node-2]
    P -->|sees source: Node-1 IP| P
```

**Problem:** Pod sees Node-1's IP, not the real client IP `203.0.113.5`.
...
graph LR
    C[Client<br/>203.0.113.5] -->|NodePort| N1[Node-1]
    N1 --> P1[Pod on Node-1]
    P1 -->|sees source: 203.0.113.5| P1
```

**Benefit:** Pod sees the real client IP. No cross-node hops.

**Risk:** If no pods exist on a node, that node returns HTTP 503. Cloud load balancers handle this via health checks — they only send traffic to nodes with healthy pods.

### When to Use `Local`

| Use Case | Why |
|----------|-----|
| **Geo-based routing** | Need real client IP for location detection |
| **Rate limiting** | Must identify real client, not internal node IPs |
| **Access logging** | Audit logs need true source IP |
| **WAF / Security** | Firewall rules depend on source IP |
| **Compliance (PCI/HIPAA)** | Must log real client IPs |
...
kubectl patch svc web-service -p '{"spec":{"externalTrafficPolicy":"Local"}}'

# Verify health check node port (auto-created with Local)
kubectl get svc web-service -o jsonpath='{.spec.healthCheckNodePort}'
# 30123 — LB uses this to check if node has pods
```

***

## Part 6: ExternalName Service
...
## Part 9: Service Troubleshooting
...
**Next note (5.3.2)** will cover **Ingress, Ingress Controllers, and Gateway API** – L7 routing, host/path-based routing, TLS termination, and the modern Gateway API.

**Backlinks:** [5.2.1 - Workloads](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (Pod labels) | [5.3.4 - CoreDNS](5.3.4_DNS_Basics_and_CoreDNS.md) (service discovery) | [Module 2 - DNS](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md) (DNS fundamentals) | [Module 2 - iptables](../../2-Networking/Subchapter_2.3/2.3.2_Firewalling_with_iptables_nftables.md) (kube-proxy)
