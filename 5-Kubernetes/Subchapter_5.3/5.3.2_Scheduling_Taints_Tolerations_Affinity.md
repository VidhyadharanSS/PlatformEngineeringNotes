# 5.3.2 Scheduling: Taints, Tolerations, and Affinity – Controlling Pod Placement
...
This note covers node selectors, taints/tolerations, node affinity, and pod affinity/anti-affinity. Note 5.3.1 covered workloads; note 5.3.3 is the subchapter review.

**Backward references:** Node concepts from 5.1.1; labels from 5.3.1; resource requests/limits from Module 4 (scheduling decisions consider available resources).
...
* Node labels from 5.3.1 (labels on pods and nodes)

* Resource requests/limits from Module 4 (scheduling considers node capacity)

* Taints on control plane from 5.2.1 (NoSchedule taint for masters)
