# 5.3.2 Ingress, Ingress Controllers, and Gateway API: L7 Routing to Services
...
This note covers Ingress resources, Ingress controllers, and the modern Gateway API. Note 5.3.1 covered Services; note 5.3.3 covers Network Policies; note 5.3.4 covers DNS/CoreDNS; note 5.3.5 is the subchapter review.

**Backlinks:** [5.3.1 - Services](5.3.1_Services_ClusterIP_NodePort_LoadBalancer.md) (Ingress routes to Services) | [Module 2 - TLS](../../2-Networking/Subchapter_2.5/2.5.2_DNS_TLS_and_Certificates_Deep_Dive.md) (certificates) | [Module 2 - DNS](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md)

***

If a LoadBalancer Service is like a phone switch routing calls by number (Layer 4), Ingress is like a hotel receptionist reading your name badge and directing you to the right conference room (Layer 7 — it understands HTTP hosts and paths).
...
> **⚠️ Advanced (skip on first pass):** Gateway API is the future of K8s networking but still maturing. Learn Ingress (above) first — it's what you'll encounter in most existing clusters. Come back here when starting a new project or when Ingress annotations become limiting.
...
## Part 9: cert-manager – Automated TLS Certificate Management

cert-manager automates TLS certificate issuance and renewal in Kubernetes. It's essential for production Ingress with HTTPS.
...
```mermaid
graph TD
    A[Ingress with annotation<br/>cert-manager.io/cluster-issuer] --> B[cert-manager Controller]
    B --> C[Create Certificate resource]
    C --> D[Create CertificateRequest]
    D --> E{Issuer Type}
    E -->|ACME| F[Let's Encrypt]
    E -->|CA| G[Internal CA]
    E -->|Vault| H[HashiCorp Vault]
    F --> I[Challenge Solver<br/>HTTP-01 or DNS-01]
    I --> J[Certificate Issued]
    J --> K[TLS Secret Created<br/>in Kubernetes]
    K --> A
```
...
spec:
  ...
  - hosts:
    - app.example.com
    secretName: app-tls-secret  # cert-manager creates this automatically
  rules:
  - host: app.example.com
    http:
      paths:
...
# app-cert   True    app-tls-secret   5m

# Describe certificate (see events, conditions)
...
| Symptom | Check | Fix |
|---------|-------|-----|
| Certificate stuck `False` | `kubectl describe certificate` | Check Issuer, DNS, challenge |
| Challenge pending | `kubectl get challenges` | Verify Ingress controller can serve `/.well-known/acme-challenge/` |
| Secret not created | `kubectl get certificaterequest` | Check cert-manager logs |
| Rate limited | Events show rate limit | Use staging issuer for testing |

***

**Next note (5.3.3)** will cover **Network Policies** – pod-to-pod isolation, ingress/egress rules, and CNI requirements.

**Backlinks:** [5.3.1 - Services](5.3.1_Services_ClusterIP_NodePort_LoadBalancer.md) | [5.3.3 - Network Policies](5.3.3_Network_Policies.md) | [5.3.4 - CoreDNS](5.3.4_DNS_Basics_and_CoreDNS.md) | [Module 2 - TLS/SSL](../../2-Networking/Subchapter_2.4/2.4.1_HTTP_HTTPS_and_curl_wget.md) | [Module 2 - DNS](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md)
