# 5.2.1 Pod Fundamentals and Lifecycle: The Atomic Unit of Kubernetes
...
This note covers Pod specifications, lifecycle, probes, and multi-container patterns. Note 5.2.2 covers Controllers (Deployments, StatefulSets, DaemonSets); note 5.2.3 covers Scheduling; note 5.2.4 is the subchapter review.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [Module 4 - Docker](../../4-Docker/Subchapter_4.1/4.1.1_Namespaces_and_Cgroups.md) | [5.4.1 - Volumes](../Subchapter_5.4/5.4.1_Ephemeral_Volumes_emptydir_hostPath.md)

> **Important:** Pods are **cattle, not pets** — they are created, killed, and replaced frequently. Never write code that depends on a specific pod's IP, name, or UID remaining stable. Use Services for stable networking and PVCs for stable storage.

> **Warning:** Running a bare Pod (not wrapped by a Deployment, StatefulSet, or Job) is almost always a mistake in production — if the node dies, the pod is gone forever with no rescheduling. Bare pods are useful only for quick debugging with `kubectl run ... --restart=Never`.
...
Init containers are like prep steps before cooking: you chop vegetables and preheat the oven (init containers) before the actual cooking (main container) starts. They run sequentially, and the main container won't start until all init containers succeed.
...
## Part 9b: Native Sidecar Containers (Kubernetes 1.28+)

Kubernetes 1.28 introduced **native sidecar containers** – init containers with `restartPolicy: Always`. This solves the long-standing problem where sidecar patterns (log collectors, service mesh proxies) had to use regular multi-container pods with no lifecycle guarantees.

### The Problem with Traditional Sidecars

```mermaid
flowchart TD
    A["Traditional sidecar<br/>(regular container)"] --> B["Starts at same time as main"]
    B --> C["No startup ordering guarantee"]
    C --> D["Main container may start<br/>before sidecar is ready"]
    D --> E["During shutdown, sidecar<br/>may die before main flushes logs"]
```

**Issues with the old approach:**
* Sidecar starts in parallel with main – no ordering
* During pod termination, sidecar may exit before main container finishes cleanup
* Jobs/CronJobs never complete because sidecar keeps running (exit code never reached)

### Native Sidecar Solution

Native sidecars are init containers with `restartPolicy: Always`. They:
* **Start before** the main containers (like init containers)
* **Keep running** throughout the pod's lifetime (like regular containers)
* **Shut down after** main containers terminate (reverse init container order)
...
spec:
  initContainers:
  # This is a native sidecar — runs for entire pod lifetime
  - name: log-collector
  ...
  # Regular init container — runs to completion, then exits
  - name: init-db
    image: busybox
...
sequenceDiagram
    participant K as Kubelet
    participant S as Sidecar (init + restartPolicy: Always)
    participant I as Init Container
    participant M as Main Container
    
    Note over K,M: Startup
    K->>S: Start sidecar (stays running)
    S-->>K: Running ✓
    K->>I: Start init container
    I-->>K: Exit 0 (completed)
    K->>M: Start main container
    
    Note over K,M: Running phase
    S->>S: Continues running alongside main
    M->>M: Application running
    
    Note over K,M: Shutdown
    K->>M: SIGTERM main container
    M-->>K: Terminated
    K->>S: SIGTERM sidecar (AFTER main exits)
    S-->>K: Terminated
```

### Key Benefits

| Feature | Traditional Sidecar | Native Sidecar |
|---------|-------------------|----------------|
| **Startup order** | Parallel with main | Before main (guaranteed) |
| **Shutdown order** | Parallel with main | After main (guaranteed) |
| **Jobs compatibility** | Blocks Job completion | Pod exits when main exits |
| **Readiness gates** | None | Sidecar must be ready before main starts |
| **Restart behavior** | Uses pod restartPolicy | Always restarts independently |

### Use Cases

| Pattern | Example |
|---------|---------|
| **Service mesh proxy** | Istio/Envoy sidecar starts before app, stops after |
| **Log collection** | Fluentd/Promtail ready before app writes logs |
| **Vault agent** | Secrets injected before app starts |
| **Jobs with sidecars** | Sidecar doesn't prevent Job completion |
...
- name: istio-proxy
  ...
  # Startup probe gates main container start
  startupProbe:
    httpGet:
      path: /healthz/ready
      port: 15021
...
> **Note:** Native sidecars require Kubernetes 1.28+ with the `SidecarContainers` feature gate enabled (GA in 1.29).
...
**Next note (5.2.2)** will cover **Workload Controllers** – Deployments, StatefulSets, DaemonSets, Jobs, and CronJobs.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [5.4.1 - Volumes](../Subchapter_5.4/5.4.1_Ephemeral_Volumes_emptydir_hostPath.md) | [5.5.1 - ConfigMaps](../Subchapter_5.5/5.5.1_ConfigMaps_and_Secrets.md)
