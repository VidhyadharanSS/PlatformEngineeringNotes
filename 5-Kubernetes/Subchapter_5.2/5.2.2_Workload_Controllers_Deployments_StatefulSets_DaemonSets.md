# 5.2.2 Workload Controllers: Deployments, StatefulSets, DaemonSets, Jobs
...
This note covers all major controllers. Note 5.2.1 covers Pod fundamentals; note 5.2.3 covers scheduling; note 5.2.4 is the subchapter review.

**Backlinks:** [5.2.1 - Pod Fundamentals](5.2.1_Pod_Fundamentals_and_Lifecycle.md) | [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [5.4.2 - PVC](../Subchapter_5.4/5.4.2_PersistentVolumes_PersistentVolumeClaims_StorageClasses.md)
...
A Deployment is like a restaurant manager who always keeps 3 chefs on shift — if one leaves (pod dies), the manager immediately hires a replacement. It also handles smooth shift changes (rolling updates) so the kitchen never stops serving.
...
StatefulSet pods are like assigned seats in a classroom (seat-0, seat-1, seat-2). Each seat has its own locker (PVC). If a student leaves, the same seat number is reassigned to the replacement — and they get the same locker back.
...
**Next note (5.2.2)** will cover **Scheduling: Taints, Tolerations, and Affinity** – controlling pod placement, node selectors, taints/tolerations, node affinity, pod affinity/anti-affinity.
