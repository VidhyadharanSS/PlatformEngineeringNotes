# 5.2.3 Scheduling: Taints, Tolerations, and Affinity – Controlling Pod Placement
...
This note covers node selectors, taints/tolerations, node affinity, and pod affinity/anti-affinity. Note 5.2.1 covers pod fundamentals; note 5.2.2 covers controllers; note 5.2.4 is the subchapter review.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [5.2.1 - Pod Fundamentals](5.2.1_Pod_Fundamentals_and_Lifecycle.md) | [5.2.2 - Controllers](5.2.2_Workload_Controllers_Deployments_StatefulSets_DaemonSets.md)
...
> **Core analogy:** A **taint** is like a "No Entry" sign on a node. A **toleration** is like a VIP pass that lets a specific pod ignore the sign. **Node affinity** answers "which node?", while **pod affinity** answers "near which other pods?"
...
## Part 8: Pod Priority and Preemption

Pod Priority and Preemption allow you to define relative importance of pods. When a cluster is full, higher-priority pods can **preempt** (evict) lower-priority pods to get scheduled.

### Why Priority Matters

* **Critical workloads survive** – Production API pods survive over batch jobs during resource pressure
* **Graceful degradation** – Non-essential pods are evicted first
* **Multi-tenant fairness** – Guarantee resources for paying tenants
* **CKA exam topic** – Frequently tested
...
```mermaid
flowchart TD
    A["High-priority pod created"] --> B{"Resources available?"}
    B -->|Yes| C["Schedule normally"]
    B -->|No| D{"Lower-priority pods exist?"}
    D -->|No| E["Pod stays Pending"]
    D -->|Yes| F["Evict lowest-priority pod(s)"]
    F --> G["Freed resources"]
    G --> C
...
**Preemption sequence:**
1. Scheduler finds no node with enough resources
2. Scheduler checks if evicting lower-priority pods would free resources
3. Selects victim pods (lowest priority first, fewest disruptions)
4. Victim pods receive graceful termination signal (respects `terminationGracePeriodSeconds`)
5. After victims terminate, high-priority pod is scheduled
...
| `system-node-critical` | 2000001000 | kubelet, container runtime static pods |
...
| `Never` | Pod waits; never evicts others (useful for batch jobs that should wait rather than disrupt) |

### Best Practices

* **Don't use very high values** – Reserve >1 billion for system pods
* **Set a globalDefault** – Ensures all pods have a defined priority
* **Use `preemptionPolicy: Never`** for batch/non-urgent workloads
* **Combine with PDBs** – PodDisruptionBudgets still respected during preemption
* **Test preemption** – Use `kubectl get events` to see preemption decisions

***

**Next note (5.2.4)** will be the Subchapter Review for Workloads and Scheduling, including a cheatsheet and scenario-based interview questions.

**Backlinks:** [5.2.1 - Workloads](5.2.1_Pod_Fundamentals_and_Lifecycle.md) (labels) | [Module 4 - Resources](../../4-Docker/Subchapter_4.3/4.3.1_Container_Lifecycle_and_Resource_Management.md) (requests/limits) | [5.9.1 - HA Architecture](../Subchapter_5.9/5.9.1_HA_Cluster_Architecture_Multi_Master.md) (control plane taints)
