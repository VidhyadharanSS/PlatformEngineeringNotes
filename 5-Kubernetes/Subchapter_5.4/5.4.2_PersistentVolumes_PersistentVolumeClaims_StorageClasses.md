# 5.4.2 PersistentVolumes, PersistentVolumeClaims, and StorageClasses: Long-Term Storage for Stateful Apps

#### Why Persistent Storage Matters

Ephemeral volumes (from 5.4.1) die with pods. Databases, message queues, and stateful applications need storage that outlives pod restarts, survives node failures, and can be moved across nodes. Kubernetes provides:
...
This note covers PV/PVC/StorageClass. Note 5.4.1 covered ephemeral volumes; note 5.4.3 is the subchapter review.

**Backlinks:** [5.2.2 - StatefulSet](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (uses PVCs) | [Module 1 - Storage](../../1-Linux/Subchapter_1.2/1.2.1_FHS_Deep_Dive.md) (NFS concepts)

***

> **Analogy:** A PV is like an available apartment (storage resource). A PVC is like a rental application (storage request). When they match (size, access mode), the apartment is "bound" to the tenant. A StorageClass is like a construction company that builds apartments on-demand when someone applies — this is dynamic provisioning.
...
### Changing PV Reclaim Policy (Prevent Data Loss)

Dynamically provisioned PVs default to `Delete` reclaim policy — meaning when you delete the PVC, the PV **and its data** are destroyed. This is a common cause of production data loss.

```bash
# Check current reclaim policy on all PVs
...
kubectl patch pv pvc-abc123-def456 -p '{"spec":{"persistentVolumeReclaimPolicy":"Retain"}}'

# Bulk change ALL PVs to Retain
kubectl get pv -o name | xargs -I{} kubectl patch {} -p '{"spec":{"persistentVolumeReclaimPolicy":"Retain"}}'

# Verify the change
kubectl get pv pvc-abc123-def456 -o jsonpath='{.spec.persistentVolumeReclaimPolicy}'
# Retain
```

**What happens after changing to Retain:**

1. When PVC is deleted, PV status changes to `Released` (not `Available`)
2. Data is preserved on the underlying storage
3. PV cannot be rebound automatically — you must manually clean it:

```bash
# After PVC deletion, PV shows Released
kubectl get pv
# NAME           CAPACITY   RECLAIM   STATUS     CLAIM
# pvc-abc123     10Gi       Retain    Released   default/my-pvc

# To reuse: remove the claimRef
kubectl patch pv pvc-abc123 -p '{"spec":{"claimRef":null}}'
# Now PV is Available for new PVCs
```

**Best practice:** Change reclaim policy to `Retain` for all production database PVs immediately after creation.
...
**Next note (5.4.3)** will be the Subchapter Review for Storage, including a cheatsheet and scenario-based interview questions.

**Backlinks:** [5.2.2 - StatefulSet](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md) (volumeClaimTemplates) | [5.4.1 - Ephemeral Volumes](5.4.1_Ephemeral_Volumes_emptydir_hostPath.md) (PVC volume type)
