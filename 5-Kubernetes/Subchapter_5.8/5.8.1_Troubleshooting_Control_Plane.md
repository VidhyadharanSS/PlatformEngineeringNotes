# 5.8.1 Troubleshooting Control Plane Components
...
This note covers diagnosing and fixing each control plane component. Note 5.8.2 covers worker node (compute plane) troubleshooting; note 5.8.3 covers monitoring.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [5.9.2 - etcd Backup](../Subchapter_5.9/5.9.2_etcd_Backup_Restore_and_Disaster_Recovery.md) | [5.9.1 - HA Architecture](../Subchapter_5.9/5.9.1_HA_Cluster_Architecture_Multi_Master.md)
...
```mermaid
flowchart TD
    A["Pod stuck in Pending"] --> B{"kubectl describe pod<br/>Events section"}
    B -->|"Insufficient cpu / memory"| C["Node capacity exhausted"]
    B -->|"untolerated taint"| D["Add toleration to pod"]
    B -->|"didn't match selector"| E["Label node or fix nodeSelector"]
    B -->|"unbound PVCs"| F["Fix StorageClass / create PV"]
    B -->|"0/N nodes available"| G["Multiple conditions blocking"]
    C --> H["Add nodes or reduce requests"]
    G --> B
...
**Next note (5.8.2)** covers **Compute Plane Troubleshooting** – worker node failures, kubelet issues, CNI problems, pod-level debugging.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) | [5.9.2 - etcd](../Subchapter_5.9/5.9.2_etcd_Backup_Restore_and_Disaster_Recovery.md) | [5.7.1 - Authentication](../Subchapter_5.7/5.7.1_Authentication_Methods.md)
