# 5.8.7 Three Pillars: Metrics, Logs, Traces

**Backlinks:** [5.9 K8s Troubleshooting](../../5-Kubernetes/) · [7.5 Nginx Monitoring](../../7-Nginx/) · [9.2 Python Logging](../../9-Python/Subchapter_9.2/)

**Next note:** [5.8.8 — SLIs, SLOs, and Alerting Philosophy](5.8.8_SLIs_SLOs_and_Alerting_Philosophy.md)
...
> **Analogy:** Imagine a filing cabinet. Each unique label combination creates a new drawer. Labels like `method=GET` and `status=200` give you a few dozen drawers — manageable. But if you add `user_id` as a label, you suddenly need one drawer per user — potentially millions. That's cardinality explosion, and it's why Prometheus runs out of memory.
...
**Do not log:**
- Passwords, tokens, API keys (see [5.7.5](../Subchapter_5.7/5.7.5_Secrets_Management_Deep_Dive.md))
...
> **⚠️ Advanced (skip on first pass):** Tail-based sampling is an optimization for high-volume production systems. Start with head-based sampling at 10% — that's enough to debug most issues. Come back here when you're handling thousands of requests per second.
...
7. **Alerting on everything.** See [5.8.8](5.8.8_SLIs_SLOs_and_Alerting_Philosophy.md) — alert on symptoms, not causes.
...
Next: [5.8.8 — SLIs, SLOs, and Alerting Philosophy](5.8.8_SLIs_SLOs_and_Alerting_Philosophy.md) — what to measure, what to promise, what to page on.
