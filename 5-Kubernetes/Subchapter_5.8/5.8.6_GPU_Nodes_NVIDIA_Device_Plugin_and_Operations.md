## 5.8.6 GPU Nodes, NVIDIA Device Plugin, and Operations

#### Why GPU Management Matters

**What are GPUs used for?** GPUs (Graphics Processing Units) excel at parallel computation — running thousands of simple calculations simultaneously. While originally designed for rendering graphics, they are now essential for machine learning (training AI models like ChatGPT), scientific simulations, and video processing. If your organization runs any ML/AI workloads, you'll be managing GPU infrastructure.

Platform engineers increasingly manage Kubernetes infrastructure for machine learning, data science, and high-performance computing. NVIDIA GPUs are the dominant accelerator, but Kubernetes cannot schedule them well until the node driver, container runtime, device plugin, labels, taints, and workload specs all agree. You will need to:

* Install and verify GPU drivers on Ubuntu worker nodes

* Monitor GPU utilization, temperature, and memory

* Ensure CUDA images can see host GPU devices through the NVIDIA container toolkit

* Troubleshoot "GPU not found" or out-of-memory errors

* Configure the NVIDIA device plugin, node labels, taints, MIG, and monitoring for GPU workloads

This note covers **NVIDIA-specific Kubernetes operations**: driver installation on Ubuntu GPU nodes, `nvidia-smi`, persistence mode, CUDA compatibility, `nvidia-container-toolkit`, the NVIDIA device plugin, MIG/time-slicing, scheduling, monitoring, and common failure modes.

> **If your infrastructure doesn't use GPUs**, this subchapter is optional. Read the summary and move to the operations review, then return when you work with ML or inference workloads.
...
## Part 3: NVIDIA Driver Installation on Ubuntu GPU Nodes

Kubernetes GPU scheduling starts below Kubernetes. The worker node must detect the GPU, load the NVIDIA kernel module, expose `/dev/nvidia*` devices, and provide driver libraries compatible with the CUDA images you run.
...
### GPU Monitoring Metrics (for Scripts and Prometheus Labels)
...
- A Kubernetes GPU node needs the host NVIDIA driver, container runtime integration, and NVIDIA device plugin before workloads can request `nvidia.com/gpu`.
- `nvidia-smi` is the first diagnostic tool: check driver/CUDA compatibility, memory, utilization, temperature, power, and active processes.
- GPU pods request GPUs through `resources.limits`; Kubernetes treats GPUs as non-overcommitted extended resources.
- Taint GPU nodes and add matching tolerations to GPU workloads so normal pods do not waste expensive accelerator nodes.
- Use MIG when you need hardware-isolated slices on supported GPUs; use time-slicing only when shared memory and weaker isolation are acceptable.
- Monitor GPU nodes with DCGM exporter, alerts for temperature/memory/error states, and workload-level metrics.
- Most GPU incidents come from driver/runtime mismatch, missing device plugin, wrong node labels, exhausted GPU capacity, or CUDA image incompatibility.

**Next note (5.8.7)** is the Kubernetes Operations review and final exam.

***

## Backlinks

- [5.2.3 Scheduling, Taints, Tolerations, Affinity](../Subchapter_5.2/5.2.3_Scheduling_Taints_Tolerations_Affinity.md) – GPU nodes rely on labels, taints, and tolerations
- [5.5.2 Autoscaling](../Subchapter_5.5/5.5.2_Autoscaling_HPA_VPA_Cluster_Autoscaler.md) – GPU node pools usually scale separately
- [5.8.2 Troubleshooting Compute Plane and Pods](./5.8.2_Troubleshooting_Compute_Plane_and_Pods.md) – debug `Pending`, `OOMKilled`, node pressure, and runtime issues
- [4-Docker Runtime Basics](../../4-Docker/) – containers need runtime integration before they can see host GPUs
