# 5.7.4 Authentication and Authorization

**Backlinks:** [2.5.1 — HTTP and REST API Design](../../2-Networking/Subchapter_2.5/2.5.1_HTTP_and_REST_API_Design.md) · [5.8 K8s RBAC](../../5-Kubernetes/) · [8.2 CI Secrets](../../8-CICD/)

**Next note:** [8.4.1 — Webhooks Done Right](../../8-CICD/Subchapter_8.4/8.4.1_Webhooks_Done_Right.md)
...
> **Analogy:** Think of a JWT like a passport. It contains your name, nationality, and photo (the claims), stamped by the government (the signature). Any border agent can read it and verify the stamp without calling the government every time. An opaque token, by contrast, is like a ticket number — the agent has to look it up in a database to know who you are.
...
**Kubernetes platform engineers:** [cert-manager](https://cert-manager.io) + a service mesh handles 90% of this for you. See [2.5.2](../../2-Networking/Subchapter_2.5/2.5.2_DNS_TLS_and_Certificates_Deep_Dive.md).
...
> **⚠️ Advanced (skip on first pass):** ABAC and ReBAC (below) are specialized patterns you'll only encounter in complex multi-tenant or document-sharing systems. RBAC (above) covers 90% of real-world needs. Come back here when RBAC feels limiting.
...
1. **`401` when you mean `403`.** They're not synonyms. See [2.5.1](../../2-Networking/Subchapter_2.5/2.5.1_HTTP_and_REST_API_Design.md#part-3).
...
3. **Symmetric JWT secret leaked in a repo.** Grep your Git history with `trufflehog`; see [8.4.2](../../8-CICD/Subchapter_8.4/8.4.2_Supply_Chain_Security_and_OWASP.md).
...
- [ ] Secrets (keys, JWT signing keys) come from a vault, not env files ([5.7.5](../Subchapter_5.7/5.7.5_Secrets_Management_Deep_Dive.md))
...
Next: [8.4.1 — Webhooks Done Right](../../8-CICD/Subchapter_8.4/8.4.1_Webhooks_Done_Right.md) — receiving HTTP callbacks, verifying HMAC signatures, idempotency, and retries.
