# 5.7.2 RBAC Deep Dive: Role-Based Access Control

#### Why RBAC Matters

> **Analogy:** RBAC is like a building access system: a **Role** is a keycard type (e.g., "floor-2 access"), a **RoleBinding** is giving that keycard to a person (Subject). **ClusterRole/ClusterRoleBinding** are master keys for the whole building.
...
This note provides a deep dive into RBAC. Note 5.7.1 covered authentication; note 5.7.3 covers Admission Controllers.

**Backlinks:** [5.7.1 - Authentication](5.7.1_Authentication_Methods.md) | [5.5.1 - Secrets](../Subchapter_5.5/5.5.1_ConfigMaps_and_Secrets.md) | [5.2.1 - ServiceAccounts](../Subchapter_5.2/5.2.1_Pod_Fundamentals_and_Lifecycle.md)

> **Important:** RBAC is **purely additive** — there are no `deny` rules. Any verb/resource combination a user has through *any* RoleBinding is allowed. To restrict access, remove bindings or scope Roles more tightly; don't try to "deny" something.

> **Tip:** Use `kubectl auth can-i <verb> <resource> --as=<user> -n <ns>` before you ship RBAC changes. It simulates authorization using the exact same code path the API server uses, so its answer is always correct.
...
**Next note (5.7.3)** will cover **Admission Controllers** – Mutating/Validating Webhooks, Pod Security Standards, and OPA Gatekeeper.

**Backlinks:** [5.7.1 - Authentication](5.7.1_Authentication_Methods.md) | [5.5.1 - ConfigMaps/Secrets](../Subchapter_5.5/5.5.1_ConfigMaps_and_Secrets.md) | [5.1.1 - API Server](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md)
