# 5.9.2 etcd Backup, Restore, and Disaster Recovery: Protecting Cluster State

> **CKA exam and production DR topic:** etcd backup/restore is critical for kubeadm clusters. On Kind/Minikube you cannot practice these commands directly. Learn the concepts now; practice on a kubeadm cluster later.
...
This note covers etcd backup/restore. Note 5.9.1 covered HA architecture; note 5.9.3 is the subchapter review.

### Backup Mental Model

etcd is the cluster's source of truth. A backup is not just a file copy; it is a consistent snapshot of the key-value database at one revision.

| Concept | Meaning | Why it matters |
| --- | --- | --- |
| Snapshot | Point-in-time copy of etcd data | lets you return the API state to a known revision |
| Revision | Monotonic etcd version number | proves when the snapshot was taken |
| Quorum | Majority of members agreeing | without quorum, etcd cannot safely serve reads/writes |
| Restore drill | Practice recovery on a disposable cluster | untested backups are only hopeful files |
| Off-cluster storage | Backup stored outside the failed cluster | protects against node, disk, and cluster-wide loss |

Treat etcd backups like database backups: encrypt them, restrict access, rotate them, and test restores. etcd may contain Secrets, service account tokens, CRDs, RBAC, and every object needed to reconstruct the cluster's state.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) (etcd) | [5.1.2 - kubeadm](../Subchapter_5.1/5.1.2_Cluster_Setup_kubeadm_Kind_Multi_Node.md) | [Module 1 - Filesystem](../../1-Linux/Subchapter_1.1/1.1.1_Linux_Filesystem_Hierarchy.md) (backup storage)
...
```bash
# Install etcdctl on Ubuntu if not present
...
## Part 4: Verifying etcd Encryption at Rest

Secrets are base64-encoded by default but **not encrypted** in etcd. Verifying encryption is a critical audit step for compliance (PCI, HIPAA, SOC2).

### Check if a Secret is Encrypted

```bash
# Read a secret directly from etcd (on control plane node)
sudo ETCDCTL_API=3 etcdctl get /registry/secrets/default/db-secret \
  --cacert=/etc/kubernetes/pki/etcd/ca.crt \
  --cert=/etc/kubernetes/pki/etcd/server.crt \
  --key=/etc/kubernetes/pki/etcd/server.key \
  | hexdump -C | head -20

# WITHOUT encryption: You'll see the secret value in plain text
# 00000050  ...password...S3cr3tPass!...

# WITH encryption (aescbc): You'll see encrypted binary data
# 00000050  k8s:enc:aescbc:v1:key1:...random binary...
```

### Verify Encryption Provider is Active

```bash
# Check API server flags for encryption config
ps aux | grep kube-apiserver | grep encryption-provider-config
# --encryption-provider-config=/etc/kubernetes/encryption-config.yaml

# View the encryption config
sudo cat /etc/kubernetes/encryption-config.yaml
# Should show: aescbc, aesgcm, or secretbox provider (not just identity)
```
...
```bash
# Re-encrypt all secrets (reads and writes back through API server)
...
sudo ETCDCTL_API=3 etcdctl get /registry/secrets/default/db-secret \
  ...
  --key=/etc/kubernetes/pki/etcd/server.key \
  | hexdump -C | head -5
# Should now show: k8s:enc:aescbc:v1:...
```

***

## Part 5: etcd Cluster Health Monitoring
...
## Part 6: Disaster Scenarios and Recovery
...
## Part 7: etcd Performance Tuning
...
## Part 8: etcdctl Command Reference
...
**Next note (5.9.3)** will be the Subchapter Review for High Availability and etcd Management, including a cheatsheet and scenario-based interview questions.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) (etcd architecture) | [5.9.1 - HA Cluster](./5.9.1_HA_Cluster_Architecture_Multi_Master.md) (multi-master) | [Module 1 - Cron](../../1-Linux/Subchapter_1.2/1.2.2_Systemd_and_Service_Management.md) (automated backups)
