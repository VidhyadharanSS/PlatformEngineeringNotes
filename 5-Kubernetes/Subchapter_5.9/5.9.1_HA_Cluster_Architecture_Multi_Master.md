# 5.9.1 HA Cluster Architecture and Multi-Master: Keeping the Control Plane Alive

> **Intermediate/Advanced:** This subchapter covers production HA setups. Read it after the workload, networking, storage, security, and operations chapters so the failure modes make sense.
...
This note covers HA architecture and multi-master setup. Note 5.9.2 covers etcd backup/restore; note 5.9.3 is the subchapter review.

### HA Mental Model

Kubernetes HA is about preserving the **control plane decision loop**, not magically preventing every workload outage.

| Plane | If it fails | HA strategy |
| --- | --- | --- |
| API server | users and controllers cannot read/write desired state | run multiple API servers behind a healthy L4 endpoint |
| etcd | cluster state cannot be read or changed | run an odd-numbered quorum and back it up |
| Scheduler | new pods stop being placed | run replicas with leader election |
| Controller manager | reconciliation loops stop | run replicas with leader election |
| Worker nodes | workloads on that node disappear or degrade | run replicas across nodes/zones with PDBs and anti-affinity |

Existing pods can keep serving when the API server is down because kubelet and the container runtime already have local instructions. But anything that needs new state, such as scaling, rollout, scheduling, or `kubectl`, depends on a healthy control plane.

**Backlinks:** [Module 2 - Load Balancing](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md) (Layer 4 LB for API servers) | [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) (etcd) | [5.1.2 - Cluster Setup](../Subchapter_5.1/5.1.2_Cluster_Setup_kubeadm_Kind_Multi_Node.md) (kubeadm)
...
## Part 8: Backup and Restore (Preview – Full details in 5.9.2)
...
**Next note (5.9.2)** will cover **etcd Backup, Restore, and Disaster Recovery** – snapshot creation, restoration procedures, and etcd cluster health management.

**Backlinks:** [5.1.1 - Architecture](../Subchapter_5.1/5.1.1_K8s_Architecture_Components.md) (etcd) | [5.1.2 - kubeadm](../Subchapter_5.1/5.1.2_Cluster_Setup_kubeadm_Kind_Multi_Node.md) | [Module 2 - Load Balancing](../../2-Networking/Subchapter_2.2/2.2.2_DNS_Deep_Dive.md)
