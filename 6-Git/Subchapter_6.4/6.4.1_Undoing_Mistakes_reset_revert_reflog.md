...
> **Analogy for `git reset` modes:** Think of committing as a 3-step assembly line: write (working dir) → stamp (staging) → ship (commit). `--soft` pulls the package back from shipping but keeps the stamp. `--mixed` removes the stamp too. `--hard` throws everything in the trash.
...
```mermaid
flowchart TD
    A["I want to undo something"] --> B{"Is the change<br/>pushed / shared?"}
    B -->|"Yes"| C["git revert<br/>(new commit that reverses)"]
    B -->|"No"| D{"Already committed?"}
    D -->|"No, only staged"| E["git restore --staged FILE"]
    D -->|"No, only in working tree"| F["git restore FILE"]
    D -->|"Yes"| G{"Keep the changes?"}
    G -->|"Yes, re-edit"| H["git reset --soft HEAD~1"]
    G -->|"Yes, but unstage"| I["git reset --mixed HEAD~1"]
    G -->|"No, throw away"| J["git reset --hard HEAD~1"]
    J --> K{"Oh no, I needed that!"}
    K -->|"Recover"| L["git reflog<br/>git reset --hard HEAD@{N}"]
