...
> **Submodule analogy:** A submodule is like a bookmark to a specific page in someone else's book. Your project says "I need version X of that library" without copying the entire book into yours.
>
> **Why is submodule removal so painful?** Git stores submodule data in three places: `.gitmodules`, the working tree, and `.git/modules/`. Each must be cleaned up separately — there's no single command that does it all.
...
LFS is like a library card catalog: Git stores a small index card (pointer) for each large file. When you need the actual file, you request it from the vault (LFS server). This keeps your repository small and fast.
