...
**Backlinks:** [6.3.1 - Rebase vs Merge](./6.3.1_Rebase_vs_Merge_and_Interactive_Rebase.md)

**Next note:** [6.3.3 - Git Hooks](./6.3.3_Git_Hooks.md)
...
- **Stash** – Temporarily save uncommitted work to switch branches (like putting your half-finished puzzle in a box so you can use the table for something else)
- **Bisect** – Binary search to find which commit introduced a bug (like finding a torn page in a 200-page book: open the middle — if it's fine, the problem is in the second half. Keep halving until you find it.)

This note covers these advanced tools. Note 6.3.1 covered rebase; note 6.3.3 covers Git hooks; note 6.3.4 is the subchapter review.
...
## Part 4: git worktree — The Modern Alternative to Stash

`git worktree` lets you check out multiple branches **simultaneously** in different directories — no stashing, no context-switching overhead. It's the modern way to work on multiple things in parallel.

### Why Worktrees Beat Stash for Context Switching

With stash, you save work → switch branches → do work → switch back → pop stash. With worktrees, both branches are checked out at the same time in separate directories.

```bash
# Create a worktree for the hotfix (checks out main in a sibling directory)
git worktree add ../hotfix-workspace main

# Now you have TWO working directories sharing the same .git:
# /project/              ← feature branch (your current work, untouched)
# /hotfix-workspace/     ← main branch (ready for hotfix)

# Fix the bug in the worktree
cd ../hotfix-workspace
git checkout -b hotfix/urgent-fix
echo "fix" >> buggy-file.js
git add . && git commit -m "fix: resolve critical bug"
git push origin hotfix/urgent-fix

# Return to your feature work (nothing was stashed or interrupted)
cd ../project
...
# List all worktrees
git worktree list
# /home/dev/project         a1b2c3d [feature/dashboard]
# /home/dev/hotfix-workspace e4f5g6h [main]

# Add a worktree (new directory, existing branch)
git worktree add ../review-pr main

# Add a worktree with a new branch
git worktree add -b hotfix/urgent ../hotfix-dir main
...
### Worktree vs Stash Comparison

| Aspect | `git stash` | `git worktree` |
|--------|-------------|----------------|
| **Context switch** | Save → switch → restore | Both branches open simultaneously |
| **Risk of forgetting** | Stash can be forgotten/lost | Visible as a directory |
| **Build cache** | Lost when switching branches | Preserved per worktree |
| **IDE support** | Single window | Open each worktree in a separate window |
| **Disk space** | Single checkout | One checkout per worktree |
| **Best for** | Quick, temporary saves | Long-running parallel work, PR reviews, bisect |

### Practical Worktree Patterns

```bash
# Pattern 1: Review a PR without leaving your work
...
# Pattern 2: Run bisect in a worktree (don't interrupt current work)
git worktree add ../bisect-workspace HEAD
cd ../bisect-workspace
git bisect start
git bisect bad HEAD
git bisect good v1.0.0
# ... bisect without touching your main checkout ...
git bisect reset
cd ../project
git worktree remove ../bisect-workspace

# Pattern 3: Build multiple branches for comparison
git worktree add ../build-main main
git worktree add ../build-feature feature/new-ui
# Build both, compare outputs side by side
```

**Key constraint:** You cannot check out the same branch in two worktrees simultaneously. Git enforces this to prevent conflicting modifications.

---

## Part 5: Combining Tools – Real-World Workflows
...
**Next note (6.3.3)** covers **Git Hooks** — automating actions at key points in the Git lifecycle: pre-commit linting, pre-push testing, commit-msg validation, and server-side hooks for CI enforcement.
