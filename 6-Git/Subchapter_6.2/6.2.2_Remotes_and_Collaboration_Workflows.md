...
**Backlinks:** [6.2.1 - Branching and Merging](./6.2.1_Branching_and_Merging_Strategies.md)

**Next note:** [6.2.3 - Tags, Signing, and Versioning](./6.2.3_Tags_Signing_and_Versioning.md)
...
This note covers remotes and collaboration workflows. Note 6.2.1 covered branching and merging; note 6.2.3 covers tags, signing, and versioning; note 6.2.4 is the subchapter review.
...
git push origin --delete feature-branch
    ...
    # Safest force push (Git 2.30+) — also checks that you've integrated remote changes
...
### The Force Push Safety Ladder

Force pushing is sometimes necessary (after rebase, amend, or history rewrite), but each option has different safety guarantees:

```bash
# ❌ DANGEROUS: Overwrites remote unconditionally
git push --force origin feature
# Blindly replaces remote — can destroy colleagues' work

# ⚠️ SAFER: Fails if remote has unexpected changes
git push --force-with-lease origin feature
# Checks that remote ref matches your last known value
# BUT: has a subtle trap (see below)

# ✅ SAFEST (Git 2.30+): Also verifies you've integrated remote changes
git push --force-with-lease --force-if-includes origin feature
# Refuses if you fetched but didn't rebase/merge the remote changes
```

**The `--force-with-lease` trap:**

`--force-with-lease` compares the remote ref against your **local tracking ref** (`origin/feature`). If you ran `git fetch` (which updates `origin/feature`) but didn't actually look at or integrate the new commits, `--force-with-lease` will **silently succeed** — overwriting those commits.

```bash
# Scenario where --force-with-lease fails to protect:
git fetch origin              # Updates origin/feature to include colleague's commit
# ... you don't look at the changes ...
git push --force-with-lease   # ✅ Succeeds! (your tracking ref is now "up to date")
# Colleague's commit is gone!

# --force-if-includes prevents this:
git push --force-with-lease --force-if-includes origin feature
# ❌ Refuses because you fetched but didn't rebase/merge the new commits
```

| Option | Checks Remote Ref | Checks Integration | Safety Level |
|--------|------------------|-------------------|-------------|
| `--force` | ❌ | ❌ | 🔴 Dangerous |
| `--force-with-lease` | ✅ | ❌ | 🟡 Mostly safe |
| `--force-with-lease --force-if-includes` | ✅ | ✅ | 🟢 Safest |

**Recommendation:** Create an alias for the safest option:
```bash
git config --global alias.fpush "push --force-with-lease --force-if-includes"
# Usage: git fpush origin feature
```
...
**Next note (6.2.3)** covers **Tags, Signing, and Versioning** — lightweight vs annotated tags, `git describe`, Semantic Versioning, Conventional Commits, and GPG/SSH commit signing for verifiable releases.
