...
> **Tip:** Open a throwaway VM before you start (or a Docker container if you've peeked at Module 4). Running `rm -rf`, editing `/etc/fstab`, and killing services is a lot less stressful on disposable infrastructure.
...
### Advanced Challenges (Tasks 14–20)

These tasks are significantly harder than 1–13. They introduce concepts not fully covered in the notes (`flock`, network namespaces, SSH fan-out). If you get stuck, that's expected — treat them as discovery exercises and revisit after Modules 4–5.
...
> **New concept:** `flock` provides file-based locking. `exec 9>"$LOCK"` opens file descriptor 9 for writing. These weren't in the notes — this is a discovery exercise.
