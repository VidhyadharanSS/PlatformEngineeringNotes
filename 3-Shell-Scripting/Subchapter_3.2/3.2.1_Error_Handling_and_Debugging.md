...
> **Important:** A script that fails loudly is safer than one that appears to succeed while silently skipping important work. Optimizing for visible failure is a production skill.
...
> **Tip:** Use distinct non-zero exit codes for different failure classes when possible. It makes automation, monitoring, and postmortems much easier.
...
A trap is like a safety net — it's code that runs automatically when something unexpected happens (an error, Ctrl+C, or script exit), ensuring your script cleans up after itself no matter how it ends.
...
The default `set -x` prefix is `+ `. Customize it with `PS4` to show **file name, line number, and function** — invaluable for debugging complex scripts.

`BASH_SOURCE` and `FUNCNAME` are built-in arrays that Bash automatically fills with the current file name and function name respectively.

```bash
#!/usr/bin/env bash

# Default PS4 is "+ " — not very useful
# Custom PS4 shows file, line, and function name
...
| PS4 Variable | Shows |
|---|---|
| `${BASH_SOURCE[0]}` | Current file name |
| `${LINENO}` | Current line number |
| `${FUNCNAME[0]}` | Current function name |
| `${SECONDS}` | Elapsed seconds (timing) |

**Timing-aware PS4 (find slow commands):**
```bash
export PS4='+ [${SECONDS}s] ${BASH_SOURCE[0]}:${LINENO}: '
```

### ShellCheck — Static Analysis for Bash

**ShellCheck** is the #1 tool for catching bash bugs before runtime. It detects quoting errors, deprecated syntax, portability issues, and common pitfalls.
...
**Common ShellCheck findings:**

| Code | Issue | Fix |
|------|-------|-----|
| SC2086 | Unquoted variable | `"$var"` instead of `$var` |
| SC2164 | `cd` without `|| exit` | `cd /dir || exit 1` |
| SC2006 | Backtick substitution | `$(cmd)` instead of `` `cmd` `` |
| SC2046 | Unquoted `$()` | `"$(cmd)"` |
| SC2034 | Unused variable | Remove or export it |
| SC2162 | `read` without `-r` | Always use `read -r` |
...
# Disable specific check inline (when you know better)
# shellcheck disable=SC2086
echo $intentionally_unquoted
```

> **Best Practice:** Run `shellcheck` on every script before committing. Treat it like a linter — zero warnings is the goal.
...
> **⚠️ Production pattern:** For learning scripts, simple `echo "[INFO] message"` to stderr is fine. This multi-level approach is useful once you maintain scripts that run unattended.
...
`>(cmd)` is *output* process substitution — it creates a "virtual file" that feeds into `cmd`. Here, `tee -a` writes to both the terminal and the log file.
...
> **📌 Reference Template — don't memorize this.** Copy it when starting a new production script. Each piece uses concepts from Parts 1–5 above.
