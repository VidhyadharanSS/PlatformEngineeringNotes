...
* Automating repetitive multi-step tasks (you'll build deployment pipelines in Module 8)
...
> **Important:** Bash is excellent for orchestration, glue code, and system automation. Once logic becomes complex or data structures get deep, consider switching to Python instead of forcing Bash to do everything.
...
> **Tip:** Prefer `$(...)` over backticks. It is easier to read, easier to nest, and less error-prone in production scripts.
...
A **subshell** is a child process that inherits variables but cannot modify the parent. Think of it like making a photocopy of your desk — you can scribble on the copy without changing the original.
...
> **⚠️ Intermediate — skip on first pass:** Process substitution is a power-user technique. Come back to it after completing 3.2 — the basics above are sufficient for now.

Process substitution makes command output appear as a file. `diff` compares two inputs line-by-line and shows what's different.
...
- [1.1.1 Kernel, OS, and Distros](../../1-Linux/Subchapter_1.1/1.1.1_Kernel_OS_and_Distros.md) - Basic navigation commands
- [1.2.2 File Types and Permissions](../../1-Linux/Subchapter_1.2/1.2.2_File_Types_Permissions_Basics.md) - `chmod +x` for scripts
- [1.6.1 Process Lifecycle](../../1-Linux/Subchapter_1.6/1.6.1_Process_Lifecycle_and_Tools.md) - Exit codes (`$?`)
