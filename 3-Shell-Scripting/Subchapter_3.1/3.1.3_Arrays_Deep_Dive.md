...
> **Tip:** Use `mapfile -t` (alias `readarray -t`) to read lines into an array. `mapfile` reads lines from stdin into an array (one element per line). `-t` trims the trailing newline from each line. It is safer than `arr=($(cmd))` because it does **not** do word splitting or globbing on the output.
...
An associative array is like a phone book — instead of looking up entry #3 (by position), you look up entry "alice" (by name). The "key" is the name you search for, and the "value" is what you get back.
...
> **⚠️ Advanced:** Namerefs are a convenience for library code. Skip on first pass — Pattern A above covers 95% of cases.
