...
A bridge is like a network switch — containers connected to it can talk to each other as if on the same LAN, but they're isolated from the outside world (unless you explicitly publish ports).
...
| **docker-proxy** | Userland process per published port | Hairpin NAT (container reaching itself via its own published host port) |
...
> **Preview (covered in Module 5):** Overlay networks are primarily used with orchestrators like Kubernetes or Swarm. Understand that they exist, but don't worry about configuring them until Module 5.

Overlay networks enable containers on different Docker hosts to communicate as if on the same network.

### Prerequisites

* Docker Swarm mode or Kubernetes (Module 5)
...
> **⚠️ Advanced (skip on first pass):** Macvlan is for legacy environments requiring containers to appear as physical devices. Most users never need this.
...
- [2.1.2 IP Addressing and Subnetting](../../2-Networking/Subchapter_2.1/2.1.2_IP_Addressing_Subnetting_CIDR.md) – Bridge subnet configuration
- [2.3.2 Firewalls and iptables](../../2-Networking/Subchapter_2.3/2.3.2_Firewalling_with_iptables_nftables.md) – Docker uses NAT for port publishing
- [2.2.1 Essential Network Tools](../../2-Networking/Subchapter_2.2/2.2.1_Essential_Networking_Tools.md) – DNS resolution concepts
