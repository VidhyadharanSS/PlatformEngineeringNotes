...
> **What is the OOM killer?** The OOM (Out of Memory) killer is a Linux kernel mechanism that forcibly terminates processes when the system (or container) runs out of memory. Exit code 137 is a strong hint you need to increase `--memory` or fix a memory leak.
...
> **⚠️ Advanced (skip on first pass):** Capabilities and seccomp are production hardening techniques. For learning, running as non-root (above) is the most important security step.

Linux capabilities split root's power into smaller permissions. By dropping all and adding back only what's needed, you follow the principle of least privilege.
