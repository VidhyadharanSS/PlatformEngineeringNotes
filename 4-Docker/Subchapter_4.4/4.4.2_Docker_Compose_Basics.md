...
```yaml
version: '3.8'  # Optional — ignored by Docker Compose v2+. Included for compatibility with older tools.
...
networks:
  ...
  backend:
    driver: bridge
    internal: true  # No external access to backend (containers can't reach the internet)
...
```mermaid
flowchart LR
    subgraph BASIC["depends_on (list form)"]
        A1["db"] --> A2["web"]
        A3["redis"] --> A2
        A4["started only — not ready"]
    end
    subgraph HEALTH["depends_on + service_healthy"]
        B1["db"] --> B2{"healthcheck<br/>passing?"}
        B2 -->|yes| B3["web starts"]
        B2 -->|no| B2
    end
...
> **Intermediate:** Profiles are useful once you have multiple environments (dev tools you don't want in production). Skip on first read.
...
> **Intermediate:** Override files become useful when you deploy to multiple environments. For learning, a single `docker-compose.yml` is sufficient.
...
> **New feature (v2.22+):** A convenient alternative to bind-mount hot reload. Requires Docker Compose v2.22 or newer.
...
> **Production topic:** Resource limits under `deploy:` are only fully enforced in Swarm mode. For standalone Compose, they work with `docker compose up` on Docker Engine 20.10+.
