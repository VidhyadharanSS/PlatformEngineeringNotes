...
## Part 3: BuildKit — Modern Build Features

BuildKit is Docker's next-generation build engine (default since Docker 23.0). It provides powerful features for faster builds, secret handling, and cache management.

### Enabling BuildKit

```bash
# Already default on Docker 23.0+. For older versions:
...
Cache package manager downloads across builds — dramatically faster rebuilds:

```dockerfile
# The `# syntax=` line tells Docker to use a specific Dockerfile parser.
...
### BuildKit Secret Mounts (`--mount=type=secret`)

> **⚠️ Advanced (skip on first pass):** Secret mounts are primarily used in automated CI/CD build pipelines (Module 8). For local builds, you can use build arguments for non-sensitive data.

Pass secrets at build time **without baking them into image layers**:

```dockerfile
# syntax=docker/dockerfile:1.4

FROM python:3.11-slim
# Secret is available during RUN but NOT stored in any layer
RUN --mount=type=secret,id=pip_conf,target=/etc/pip.conf \
    pip install -r requirements.txt

# Access private Git repos during build
...
```bash
# Build with secrets (file-based)
docker build --secret id=pip_conf,src=./pip.conf \
             --secret id=github_token,src=./token.txt \
             -t myapp .

# Secret NEVER appears in docker history — unlike ENV or ARG
docker history myapp  # No trace of secret values
```

### BuildKit SSH Mounts (`--mount=type=ssh`)

Forward SSH agent for private repo access during builds:
...
### BuildKit Feature Summary

| Feature | Purpose | Without BuildKit |
|---------|---------|-----------------|
| `--mount=type=cache` | Cache package downloads | Slow rebuilds every time |
| `--mount=type=secret` | Build-time secrets | Secrets leaked in layers (via `ARG`/`ENV`) |
| `--mount=type=ssh` | SSH agent forwarding | Copy SSH keys into image (security risk) |
| Parallel builds | Build independent stages in parallel | Sequential execution |
| `--output` | Export build artifacts without image | `docker cp` from container |

***

## Part 4: Multi-Stage Builds – The Game Changer

Multi-stage builds are like a factory: Stage 1 is the assembly line (compilers, build tools everywhere), Stage 2 is the showroom (only the finished product). You get a tiny final image without any build-time clutter.
...
## Part 5: Layer Optimization Techniques
...
> **Enterprise topic:** Content Trust is relevant for production registries, not local development. Skip on first pass.
...
## Part 7: Production-Ready Multi-Stage Example
...
## Part 8: Image Size Analysis Tools
...
- [1.1.2 Package Management](../../1-Linux/Subchapter_1.7/1.7.2_DPKG_and_APT.md) – apt, apk commands
