...
> **Warning:** Adding a user to the `docker` group is effectively granting root-equivalent access on that host, because the Docker daemon can mount the filesystem and start privileged containers.
...
### Docker Context — Managing Multiple Docker Endpoints

> **⚠️ Advanced (skip on first pass):** Contexts are useful when managing staging/production Docker hosts remotely. For local learning, you only need the default context.

Docker contexts let you manage multiple Docker daemons (local, remote servers, cloud instances) from a single CLI.

```bash
# List contexts (default is the local daemon)
docker context ls
# NAME        DESCRIPTION                               DOCKER ENDPOINT
# default *   Current DOCKER_HOST based configuration    unix:///var/run/docker.sock

# Create context for remote Docker host (via SSH)
docker context create staging --docker "host=ssh://user@staging.example.com"

# Create context for TCP endpoint
docker context create prod --docker "host=tcp://prod-docker.example.com:2376,ca=~/.docker/ca.pem,cert=~/.docker/cert.pem,key=~/.docker/key.pem"

# Switch to remote context
docker context use staging

# Now ALL docker commands target the staging server
docker ps        # Lists containers on staging
docker images    # Lists images on staging

# Switch back to local
docker context use default

# One-off command on different context (without switching)
...
| `docker context create` | Add new endpoint |
| `docker context use` | Switch active context |
| `docker context rm` | Remove a context |
| `docker context inspect` | Show context details |

### Docker Behind a Corporate Proxy

> **Enterprise environment only:** Skip unless Docker pulls fail behind a corporate network.

In enterprise environments, Docker needs proxy configuration for pulling images and building.

```bash
# Method 1: Daemon-level proxy (for image pulls)
sudo mkdir -p /etc/systemd/system/docker.service.d
sudo tee /etc/systemd/system/docker.service.d/http-proxy.conf << 'EOF'
[Service]
Environment="HTTP_PROXY=http://proxy.corp.com:8080"
Environment="HTTPS_PROXY=http://proxy.corp.com:8080"
Environment="NO_PROXY=localhost,127.0.0.1,.corp.com,10.0.0.0/8"
EOF
sudo systemctl daemon-reload
sudo systemctl restart docker

# Method 2: Client-level proxy (for builds)
# ~/.docker/config.json
...
> **Tip:** If image pulls work on your laptop but fail in corporate environments, suspect proxy settings, internal CA certificates, or outbound firewall rules before blaming Docker itself.

### Docker vs Podman (Alternative)

| Feature | Docker | Podman |
|---------|--------|--------|
| **Daemon** | Requires dockerd | Daemonless (no background service, reducing attack surface) |
...
> **Note:** You only need to touch `daemon.json` for special cases. The defaults work perfectly for learning. Skim this section and return when you need to tune something specific.
...
- [1.6.2 Systemd Deep Dive](../../1-Linux/Subchapter_1.6/1.6.2_Systemd_Deep_Dive.md) – Starting Docker as a service
- [2.2.1 Essential Network Tools](../../2-Networking/Subchapter_2.2/2.2.1_Essential_Networking_Tools.md) – Port publishing relates to networking
- [1.6.1 Process Lifecycle](../../1-Linux/Subchapter_1.6/1.6.1_Process_Lifecycle_and_Tools.md) – `docker top` shows processes
