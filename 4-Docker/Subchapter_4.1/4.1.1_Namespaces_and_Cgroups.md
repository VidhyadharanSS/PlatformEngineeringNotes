...
Think of namespaces like separate apartments in a building — each has its own address, its own keys, and its own rooms, but they all share the same physical structure (the kernel). A process in one "apartment" can't see or interfere with processes in another.
...
## Part 4: Cgroups v1 vs v2 — Understanding the Transition

> **⚠️ Advanced (skip on first pass):** This section covers cgroup version differences relevant for production troubleshooting. For learning Docker, the commands in Part 3 are all you need.

Modern Linux distributions are migrating from cgroups v1 to **cgroups v2** (unified hierarchy). This impacts how Docker and Kubernetes manage resources.
...
| **Filesystem** | `/sys/fs/cgroup/<controller>/` | `/sys/fs/cgroup/` (flat) |
| **Memory accounting** | Separate memory + memsw | Unified `memory.max`, `memory.swap.max` |
| **CPU controller** | `cpu.cfs_quota_us` / `cpu.cfs_period_us` | `cpu.max` (single file: `"quota period"`) |
| **Default on** | Ubuntu ≤ 20.04, RHEL ≤ 8 | Ubuntu ≥ 22.04, RHEL ≥ 9, Fedora ≥ 31 |
| **Docker support** | Full | Full (Docker 20.10+) |
| **K8s support** | Full | Full (K8s 1.25+) |

### Detecting Your Cgroup Version
...
# Docker also reports it
docker info | grep -i cgroup
# Cgroup Driver: systemd
# Cgroup Version: 2
```

### Cgroups v2 Resource Files

```bash
# v1: separate directories per controller
cat /sys/fs/cgroup/memory/docker/<id>/memory.limit_in_bytes
cat /sys/fs/cgroup/cpu/docker/<id>/cpu.cfs_quota_us

# v2: unified directory, all controllers in one place
cat /sys/fs/cgroup/system.slice/docker-<id>.scope/memory.max
cat /sys/fs/cgroup/system.slice/docker-<id>.scope/cpu.max
# cpu.max output: "200000 100000" (quota period) = 2 CPUs
```

### The 7th Namespace: Time (Linux 5.6+)

> **Trivia / Advanced:** Not required for day-to-day Docker usage. Included for completeness.

Linux kernel 5.6 added the **Time namespace**, bringing the total to **seven**:

| Namespace | Kernel | Purpose |
|-----------|--------|---------|
| PID, NET, MNT, UTS, IPC, USER | 3.8+ | Standard container isolation |
| **TIME** | **5.6+** | Isolate `CLOCK_MONOTONIC` and `CLOCK_BOOTTIME` — container sees different uptime than host |

```bash
# Check if time namespace is supported
ls /proc/self/ns/time_for_children 2>/dev/null && echo "Time NS supported"
```

> **Why this matters:** Cgroups v2 is required for some K8s features like **MemoryQoS** and **Pod overhead accounting**. If you're on an older system, plan the migration.

***

## Part 5: Hands-On – Exploring Namespaces and Cgroups
...
## Part 6: Why This Matters for Platform Engineers
...
- [1.6.1 Process Lifecycle](../../1-Linux/Subchapter_1.6/1.6.1_Process_Lifecycle_and_Tools.md) – PID namespace relates to `ps`, `kill`
...
- [1.2.1 FHS Deep Dive](../../1-Linux/Subchapter_1.2/1.2.1_FHS_Deep_Dive.md) – MNT namespace relates to mount points
