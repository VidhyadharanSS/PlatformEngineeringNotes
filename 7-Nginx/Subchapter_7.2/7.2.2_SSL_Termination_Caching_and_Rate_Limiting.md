...
> **⚠️ Advanced (skip on first pass):** OCSP stapling, session tickets, and 0-RTT below are production SSL optimizations. The basic SSL config above is sufficient for getting started.
...
### SSL Session Tickets and TLS 1.3 0-RTT

**Session resumption** lets returning visitors skip the full TLS handshake. There are two mechanisms:

| Mechanism | How It Works | State Storage |
|-----------|-------------|---------------|
| **Session Cache** (`ssl_session_cache`) | Server stores session in shared memory; client sends session ID | Server-side (memory) |
| **Session Tickets** (`ssl_session_tickets`) | Server encrypts session into a ticket; client stores it and sends back | Client-side (no server memory) |

```nginx
server {
    listen 443 ssl http2;
    
    # Session cache (server-side memory — already covered above)
    ssl_session_cache shared:SSL:10m;
    ssl_session_timeout 10m;
    
    # Session tickets (client-side — no server memory needed)
    ssl_session_tickets on;        # Default: on
    ssl_session_ticket_key /etc/nginx/ssl/ticket.key;  # Optional: custom key for multi-server
}
```

**Multi-server session ticket keys:** When load balancing across multiple Nginx instances, each server generates a different ticket key by default — so a session established on server A can't be resumed on server B. Fix: share the same ticket key file:

```bash
# Generate shared ticket key (rotate every 24h)
openssl rand 80 > /etc/nginx/ssl/ticket.key
chmod 600 /etc/nginx/ssl/ticket.key
# Distribute to all Nginx instances
```

```nginx
# All Nginx instances use the same key
ssl_session_ticket_key /etc/nginx/ssl/ticket.key;
```

**TLS 1.3 Early Data (0-RTT):**

TLS 1.3 supports **0-RTT resumption** — returning clients can send application data in the very first TLS message, eliminating one round-trip entirely. However, 0-RTT data is vulnerable to **replay attacks** (an attacker can re-send the captured first message).
...
server {
    ...
    ssl_early_data on;   # Enable 0-RTT (TLS 1.3 only)
    
    # Tell the backend if the request came via 0-RTT (so it can reject unsafe replays)
    proxy_set_header Early-Data $ssl_early_data;
}
```

| Value of `$ssl_early_data` | Meaning |
|---------------------------|---------|
| `""` (empty) | Normal TLS handshake (safe) |
| `"1"` | Request was sent as 0-RTT early data (replay-vulnerable) |

> **Security rule:** Only enable `ssl_early_data` for **idempotent requests** (GET, HEAD). Backends receiving `Early-Data: 1` should reject non-idempotent methods (POST, PUT, DELETE) to prevent replay attacks. Never enable 0-RTT for payment or authentication endpoints.
...
Proxy caching is like keeping photocopies of frequently requested documents at the front desk — instead of walking to the archive room (backend) every time, you hand over the copy instantly. The copy is refreshed periodically to stay current.
...
Rate limiting uses a "leaky bucket" model — requests fill a bucket at the defined rate (`rate=10r/s`); `burst` is the bucket size (extra capacity); overflow is rejected with 503. `nodelay` processes burst requests immediately instead of queuing them.
...
### Two-Stage Rate Limiting with `delay` — The Production Pattern

`nodelay` processes the entire burst immediately — but this means during a spike, the backend receives `burst + rate` requests at once. `delay` creates a **two-stage model**: requests up to `delay` are processed immediately; requests between `delay` and `burst` are queued and released at the defined `rate`.
...
**How the three modes compare (rate=10r/s, burst=20):**

| Mode | Requests 1–10 | Requests 11–20 | Requests 21+ |
|------|--------------|----------------|--------------|
| **No burst** | Processed immediately | **Rejected (503)** | Rejected |
| **`burst=20 nodelay`** | Processed immediately | Processed immediately | **Rejected (503)** |
| **`burst=20 delay=10`** | Processed immediately | **Queued**, released at 10r/s | Rejected (503) |

```
With delay=10:

Requests:  |||||||||| |||||||||| ||
           ← instant → ← queued → ← rejected
           (1-10)      (11-20)     (21+)
```

**Why `delay` is better than `nodelay` for production:**
- `nodelay`: A 20-request burst hits the backend all at once → potential backend overload
- `delay=10`: First 10 hit instantly, next 10 are smoothed to 10r/s → backend sees steady traffic
- Result: better backend stability, same client experience for legitimate users

> **Rule of thumb:** Set `delay` to 50% of `burst` for API endpoints. For login endpoints, use low `burst` with no `delay` (you want to reject brute-force attempts, not queue them).
