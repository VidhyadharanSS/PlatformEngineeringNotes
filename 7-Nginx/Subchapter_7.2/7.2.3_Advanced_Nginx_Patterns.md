...
The `http` block is like a multilingual interpreter (understands HTTP headers, methods, paths). The `stream` block is like a phone switchboard — it connects callers to extensions without understanding the conversation. Use it for databases, Redis, game servers, or any non-HTTP protocol.
...
> **What is FastCGI?** FastCGI is a protocol for web servers to communicate with application processors. Unlike old CGI (which spawned a new process per request), FastCGI keeps processes alive — hence "Fast". PHP-FPM (FastCGI Process Manager) is PHP's implementation.
...
## Part 5: `mirror` — Traffic Shadowing

> **⚠️ Advanced (skip on first pass):** Traffic mirroring is a DevOps pattern for pre-deployment validation. Learn basic proxy_pass first.

`mirror` sends a **copy** of each request to a secondary backend without affecting the client response. The mirrored request's response is discarded. This is invaluable for:

- **Pre-deployment validation** — shadow production traffic to a new version before switching
- **Load testing** — replay real traffic to a test environment
- **Analytics** — capture a copy of every request for analysis
...
server {
    ...
    location / {
        mirror /mirror;               # Send a copy to the /mirror location
        mirror_request_body on;       # Include request body in the copy (default: on)

        proxy_pass http://production_backend;
    }

    location = /mirror {
        internal;                     # Not accessible directly by clients
...
**How it works:**
1. Client sends `POST /api/orders` → Nginx proxies to `production_backend`
2. Simultaneously, Nginx sends an identical `POST /api/orders` to `test_backend`
3. Client receives the response from `production_backend` only
4. `test_backend`'s response is silently discarded

> **Warning:** Mirror copies **all** requests including POSTs. If the mirror backend writes to a real database, you'll get duplicate data. Always point `mirror` at a **read-only** or **disposable** environment.

### Mirror Multiple Backends
...
location = /mirror_v2 {
    internal;
    proxy_pass http://staging_v2$request_uri;
}
...
## Part 6: gRPC Reverse Proxy (`grpc_pass`)

> **⚠️ Advanced (skip on first pass):** gRPC proxying is specialized for microservice architectures using Protocol Buffers. Skip this if you haven't worked with gRPC services.

gRPC is Google's high-performance RPC framework for service-to-service communication. It uses HTTP/2 and Protocol Buffers for efficient binary serialization. Nginx can reverse-proxy gRPC traffic using `grpc_pass` — similar to `proxy_pass` but speaks HTTP/2 to the backend.

> **Requirement:** Nginx 1.13.10+ with `--with-http_v2_module` (included in all official packages).

### Basic gRPC Proxy

```nginx
server {
    listen 443 ssl http2;     # gRPC clients connect over HTTP/2
    ...
    location / {
        grpc_pass grpc://localhost:50051;     # Backend gRPC server (plaintext)
        # grpc_pass grpcs://localhost:50051;  # Backend gRPC server (TLS)
    }
}
```
...
### `grpc_pass` vs `proxy_pass` for HTTP/2

| Directive | Protocol to Backend | Use Case |
|-----------|-------------------|----------|
| `proxy_pass http://` | HTTP/1.1 | REST APIs, web apps |
| `proxy_pass http://` + `proxy_http_version 1.1` | HTTP/1.1 with keepalive | REST with connection reuse |
| `grpc_pass grpc://` | HTTP/2 (plaintext) | gRPC without backend TLS |
| `grpc_pass grpcs://` | HTTP/2 (TLS) | gRPC with backend TLS |

**Kubernetes connection:** Every Kubernetes API call (kubectl → API server → etcd, kubelet, etc.) uses gRPC. If you're debugging Kubernetes networking, understanding gRPC proxying through Nginx helps diagnose ingress issues for gRPC-based microservices.

> **Common mistake:** Using `proxy_pass` for gRPC traffic. `proxy_pass` speaks HTTP/1.1 to backends, but gRPC requires HTTP/2 framing. You'll get `502 Bad Gateway` or garbled responses. Always use `grpc_pass` for gRPC backends.

---

## Part 7: Debugging Nginx
...
## Part 8: `resolver` Directive — DNS for Upstream
