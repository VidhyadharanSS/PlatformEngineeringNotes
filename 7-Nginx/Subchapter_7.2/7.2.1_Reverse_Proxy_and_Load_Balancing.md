...
> **Tip:** Always set `proxy_set_header X-Real-IP $remote_addr;` and `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` when proxying. Without them, your backend sees **Nginx's** IP as the client IP, breaking rate limiting, audit logs, and geo features.

> **Warning:** Nginx's default `proxy_read_timeout` is 60 s. Long-polling APIs, WebSockets, and streaming responses will be cut off — bump it to a sensible value (or `0` for no timeout) only on the specific `location` blocks that need it.

---

## Part 1: What is a Reverse Proxy?

A reverse proxy is like a company's front desk — visitors (clients) don't know which specific employee (backend server) handles their query; the front desk routes them to the right person and relays the answer back.
...
An `upstream` block is like defining a team roster — Nginx knows all available players and picks who handles the next request based on the chosen strategy (round-robin, least connections, IP hash, etc.).
