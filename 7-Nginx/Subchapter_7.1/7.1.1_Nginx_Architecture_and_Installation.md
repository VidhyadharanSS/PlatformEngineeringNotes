...
**Backlinks:** [Module 1 — Linux (systemd, packages, file permissions)](../../1-Linux/) | [Module 2 — Networking (TCP, ports, HTTP)](../../2-Networking/) | [Module 4 — Docker (container networking)](../../4-Docker/) | [Module 5 — Kubernetes (Services, Ingress)](../../5-Kubernetes/)

**Next note:** [7.1.2 — Static File Serving and Location Matching](./7.1.2_Static_File_Serving_and_Location_Matching.md)
...
> **Analogy:** Think of a restaurant: Apache hires a dedicated waiter per table (expensive — 10,000 tables need 10,000 waiters). Nginx has one efficient waiter who checks every table in rotation — same service, far fewer staff. That's the event-driven model.
...
### Why Event-Driven Is Faster — Key Concepts Explained

| Term | Explanation |
|------|-------------|
| **epoll** | Linux kernel event notification interface. Nginx registers all sockets with epoll and gets notified when data is ready — no polling/waiting. Much more efficient than `select` or `poll` at scale. |
| **multi_accept on** | Each worker accepts ALL new connections at once when the OS signals availability, instead of accepting one at a time. Reduces latency under bursty traffic. |
| **Non-blocking I/O** | Nginx does not wait for a slow client or backend — it registers the socket with epoll, handles other requests, and resumes when data arrives. |
...
## Part 3: CPU Affinity and NUMA Tuning

> **⚠️ Advanced (skip on first pass):** The section below covers CPU pinning and NUMA topology — performance tuning for high-traffic bare-metal servers. On VMs, containers, or any setup handling < 10,000 req/s, the defaults are fine. Skip to Part 4 (Configuration Structure) for now.

After setting `worker_processes auto`, the next performance lever is **binding each worker to a specific CPU core**. Without affinity, the OS scheduler can migrate workers between cores, causing **L1/L2 cache misses** that add microseconds to every request — multiplied by millions of requests, this becomes significant.

### `worker_cpu_affinity` — Pin Workers to Cores

```nginx
# Auto-assign workers to CPU cores (recommended — Nginx 1.9.10+)
worker_processes auto;
worker_cpu_affinity auto;

# Manual affinity for a 4-core machine (bitmask per worker):
worker_processes 4;
worker_cpu_affinity 0001 0010 0100 1000;   # Worker 0→core 0, 1→core 1, etc.

# 8-core machine, only use first 4 cores (leave 4 for backend app):
worker_processes 4;
...
|-----------------|---------------|
| OS migrates worker between cores | Worker stays on one core |
| L1/L2 cache is cold after migration | Cache stays warm (hot path) |
| ~5-15% throughput loss at high concurrency | Consistent, predictable latency |

### NUMA-Aware Tuning (Multi-Socket Servers)

On servers with multiple physical CPUs (NUMA architecture), workers that access memory from a remote NUMA node pay a ~50ns penalty per access. Bind workers to cores on the same NUMA node as the network card:
...
# Check which NUMA node owns the NIC
cat /sys/class/net/eth0/device/numa_node
# 0
```

```nginx
# Bind all workers to NUMA node 0 (where the NIC lives)
worker_processes 8;
worker_cpu_affinity 00000001 00000010 00000100 00001000 00010000 00100000 01000000 10000000;
```

> **When to care:** Only on bare-metal or dedicated servers with >8 cores. In containers (Docker, K8s), the container runtime handles CPU pinning via `cpuset` — set `worker_cpu_affinity auto` and let the orchestrator manage cores.

---

## Part 4: Nginx Configuration Structure

> **Config hierarchy analogy:** Think of it like a company org chart: `main` context is the CEO (global settings), `events` is HR (connection handling), `http` is the web department, each `server` block is a team (virtual host), and each `location` is a team member handling specific URL paths.
...
worker_cpu_affinity auto;        # Pin each worker to a core (reduces cache misses)
worker_rlimit_nofile 65536;      # Max open files per worker (a "file descriptor" is how Linux tracks every open file, socket, or connection — each request needs at least one)
pid /run/nginx.pid;
...
http {
    ...
    # Open file cache (reduces syscalls for frequently served files)
...
## Part 5: Key Configuration Directives
...
| `worker_rlimit_nofile` | Max file descriptors per worker — set to ≥ `worker_connections * 2` | `65536` |
...
| `gzip` | Compression | `on` |
| `open_file_cache` | Cache file descriptors & metadata in memory | `max=10000 inactive=30s` |
| `open_file_cache_valid` | How often to re-check cached entries | `60s` |
...
## Part 6: Testing and Reloading Configuration
...
**Next note:** [7.1.2 — Static File Serving and Location Matching](./7.1.2_Static_File_Serving_and_Location_Matching.md) — `root` vs `alias`, `try_files`, location matching rules, and logging.
