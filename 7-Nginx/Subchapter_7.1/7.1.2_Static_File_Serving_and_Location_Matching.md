...
Think of it like a mail sorting office: exact address matches are sorted first, then zip code prefixes, then pattern matching — most specific always wins.

> **Regex basics for Nginx:** The `~` prefix means "regex match" (`~*` is case-insensitive). Common patterns: `\.` matches a literal dot, `$` means end of string, `()` captures groups, `\d+` matches digits.
...
### ETags and Conditional Requests (304 Responses)

Caching headers (`expires`, `Cache-Control`) tell the browser **how long** to cache. But what happens when that time expires? The browser doesn't re-download the full file — it sends a **conditional request** asking "has this file changed?" If not, the server returns `304 Not Modified` (zero-byte body), and the browser uses its cached copy.

Nginx handles this automatically using two mechanisms:

| Mechanism | Header Sent by Nginx | Browser Sends Back | How Nginx Validates |
|-----------|---------------------|-------------------|-------------------|
| **ETag** | `ETag: "65a3f2b0-1a4f"` | `If-None-Match: "65a3f2b0-1a4f"` | Compares ETag values |
| **Last-Modified** | `Last-Modified: Thu, 15 Jan 2024 10:30:00 GMT` | `If-Modified-Since: Thu, 15 Jan 2024 10:30:00 GMT` | Compares timestamps |

```nginx
# Nginx enables ETags by default — to verify:
location ~* \.(css|js|jpg|png)$ {
    etag on;                      # Default: on (generates ETag from file mtime + size)
    if_modified_since exact;      # Default: exact (compares timestamps exactly)
    expires 30d;
}

# Disable ETags (rare — only for load-balanced backends with different inodes)
location /cdn/ {
    etag off;
}
```

**How Nginx generates ETags:** The default ETag is a hex string combining the file's `mtime` (modification time) and `size`. Format: `"<hex-mtime>-<hex-size>"`. This means:
- Two identical files on **different servers** will have the **same ETag** if `mtime` and `size` match
- Deploying the same file to multiple servers produces matching ETags (unlike Apache, which uses inode numbers)

```bash
# Test conditional request flow:
# First request — full response (200)
curl -I https://example.com/style.css
# ETag: "65a3f2b0-1a4f"
# Last-Modified: Thu, 15 Jan 2024 10:30:00 GMT

# Second request — conditional (304, no body transferred)
curl -I -H 'If-None-Match: "65a3f2b0-1a4f"' https://example.com/style.css
...
| Value | Behaviour |
|-------|-----------|
| `exact` (default) | Return 304 only if timestamps match exactly |
| `before` | Return 304 if file was modified **at or before** the `If-Modified-Since` time — more lenient, useful behind CDNs |
| `off` | Ignore `If-Modified-Since` headers entirely |

> **When to disable ETags:** In load-balanced setups where Nginx serves files from different filesystem paths (different inodes/mtimes). If ETags don't match across backends, browsers re-download on every request routed to a different server. The fix is either `etag off` or ensuring identical deploy timestamps.
