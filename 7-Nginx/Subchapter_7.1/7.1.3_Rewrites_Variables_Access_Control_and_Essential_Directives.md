...
Think of `map` as a lookup table or dictionary — given an input key, it returns the corresponding value, but only computes it when someone actually asks for it (lazy evaluation).
...
> **Why "if is evil":** Using `if` inside a location is like writing two overlapping contracts — only one takes effect, and you can't predict which clauses survive. Nginx internally creates an implicit nested location for each `if`, causing directives to silently disappear.
